function Dualprocess7_2RNA_time(path)
tic
...
mismatch_path='D:\Eyedropper\Standard\';
fast_add = 'F';
airy_add = 'A';
double_add = 'D';
...
% pathname=replace(path,'Duallist.xls','');
...
for list_I = 1:N1
    ...
    try
        [sub_num, sub_list] = xlsread([folder_name,in_folder,input_name]);
    catch
    ...
    if ~isempty(out_folder0)
        copyfile([folder_name,out_folder],[folder_name,out_folder0]);
    end
    ...
    for list_J = 1:M1%run_list{list_I}
        if isempty(strfind(sub_list{list_J,3},'_60X')) && isempty(strfind(sub_list{list_J,2},'60X')) && isempty(strfind(sub_list{list_J,2},airy_add)) && isempty(strfind(sub_list{list_J,2},fast_add))
            [~,~,mismatch_matrix] = xlsread([pathname,mismatch_name]);
            load([mismatch_path,xymismatch_name])
            mismatch_name
        elseif (~isempty(strfind(sub_list{list_J,3},'_60X')) || ~isempty(strfind(sub_list{list_J,2},'60X'))) && isempty(strfind(sub_list{list_J,2},airy_add)) && isempty(strfind(sub_list{list_J,2},fast_add))
            [~,~,mismatch_matrix] = xlsread([pathname,mismatch_name2]);
            load([mismatch_path,xymismatch_name2])
            mismatch_name2
        elseif (~isempty(strfind(sub_list{list_J,3},'_60X')) || ~isempty(strfind(sub_list{list_J,2},'60X'))) && ~isempty(strfind(sub_list{list_J,2},fast_add))
            [~,~,mismatch_matrix] = xlsread([pathname,mismatch_name3]);
            load([mismatch_path,xymismatch_name3])
            mismatch_name3
        else
...
%         image_folder = [folder_name,in_folder,sub_list{list_J,3}];
        image_folder = [folder_name,in_folder,sub_list{list_J,3}(1:end-1),old_add,'/'];
        ...
        for time_index=1:time_num
            ...
            output_tail =[time_name,num2str(time_index,'%04u'),'.xls'];
            ...
            load([folder_name,mask_folder,sub_list{list_J,3},mask_name]);   %%% load 3D mask
            mask_stack = double(mask_stack);
            z_size = size(mask_stack,3);
            load([folder_name,fit_folder,sub_list{list_J,3}(1:end-1),fit_add,mat_tail]);   %%% load spot intensity fitting result
    % %         b = 1;
            Inten_thresh = b*N_thresh;   %%% set foci intensity threshold
            single_Inten = b;
            load([folder_name,fit_folder2,sub_list{list_J,3}(1:end-1),fit_add,mat_tail]);   %%% load spot intensity fitting result
            Inten_thresh2 = b*N_thresh2;   %%% set foci intensity threshold
            single_Inten2 = b;

            load([folder_name,fit_folder_protein,sub_list{list_J,3}(1:end-1),fit_add,mat_tail]);   %%% load spot intensity fitting result
            Inten_protein = b;
    ...
    %         N_cycle = sub_num(list_J,13);
            ...
            [foci_bw3D,max_image00,SS] = modified_foci3D([folder_name,hist_folder,sub_list{list_J,3}(1:(end-1)),hist_tail],max_image,RNA_channel,mask_stack,N_cycle,image_folder,protein_RNA_mismatch,Inten_thresh,[],resolution,use_linear);
    ...
    % %         flip_EL = EL_orientation(nucleus_DAPI_profile,EL_info,mask_stack,eval(EL_check_name),flip_axis,[],resolution);
            flip_EL = EL_orientation2(EL_info,mask_stack,[folder_name,hist_folder,sub_list{list_J,3}(1:(end-1)),hist_tail],protein_RNA_mismatch,flip_axis,[]);
    ...
    %         signal_stack = signal_stack/quanti_p(1);
            ...
            [foci_bw3D,max_image00,SS] = modified_foci3D2([folder_name,hist_folder,sub_list{list_J,3}(1:(end-1)),hist_tail],max_image,RNA_channel,mask_stack,N_cycle,image_folder,protein_RNA_mismatch,protein_RNA_xymismatch,Inten_thresh,[],resolution,{x_start_im,y_start_im,x_center_im,y_center_im},use_linear);
            max_image00 = max_image00/single_Inten;
    ...
    %         clear RNA_stack
            ...
            [foci_bw3D2,max_image002,SS2] = modified_foci3D([folder_name,hist_folder2,sub_list{list_J,3}(1:(end-1)),hist_tail],max_image,signal2_channel,mask_stack,N_cycle,image_folder,protein_signal2_mismatch,Inten_thresh2,44,resolution,use_linear);
            ...
            [foci_bw3D2,max_image002,SS2] = modified_foci3D2([folder_name,hist_folder2,sub_list{list_J,3}(1:(end-1)),hist_tail],max_image,signal2_channel,mask_stack,N_cycle,image_folder,protein_signal2_mismatch,protein_signal2_xymismatch,Inten_thresh2,44,resolution,{x_start_im,y_start_im,x_center_im,y_center_im},use_linear);
            max_image002 = max_image002/single_Inten2;
    ...
    try
        ...
        xlwrite([folder_name,in_folder,input_name],sub_list);
    end
end
toc