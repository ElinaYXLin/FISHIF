function [x_shift,y_shift,rotate_angle]=im_rectify2(im1_path,im2_path)
...
stack1L=stack1>mean(stack1(:));
stack1F=imfilter(double(stack1L),ones(50,50));
% stack2_roF=imfilter(stack2_ro,ones(50,50));
[~,Index]=max(stack1F(:));
[L1,L2]=size(stack1);
[y1,x1]=ind2sub(size(stack1F),Index);
Yshift=round(L1/2)-y1;
Xshift=round(L2/2)-x1;
stack1=circshift(stack1,[Xshift Yshift]);
stack2_ro=circshift(stack2_ro,[Xshift Yshift]);
%% INI
scan_reg=0.4;
bin_scan=2;
