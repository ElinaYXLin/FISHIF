...
%                     th_estimate = 1.2*sum(th_center2.*exp(-center_dist2/2/sigma_th^2))/sum(exp(-center_dist2/2/sigma_th^2));
                    th_estimate = 0.8*sum(th_center2.*exp(-center_dist2/2/sigma_th^2))/sum(exp(-center_dist2/2/sigma_th^2));
                    bw_th = bwareaclose(im2bw(fil_im,th_estimate),merge_th);
%                     bw_th = imfill(bwareaclose(im2bw(im_temp,th_estimate),merge_th)wo,'holes');
% %                     bw_th = im2bw(im_temp,th_estimate);
                    bw_th_label = bwlabel(bw_th);
                    label_select = setdiff(unique(bw_th_label(bw_diff_label == bw_diff_label(bw_center0(I_center)))),0);
                    bw_I = ismember(bw_th_label,label_select);% & bw_th;
                    bw_out = bw_out | bw_I;
                    th_2D(bw_I) = max(th_2D(bw_I),th_estimate);
                end
% % %                 bw_out = imopen(bw_out,strel('disk',3));
...
% %             for n0 = [4,2]
% %                 bw_cut = imerode(bw_out,ellipse_mask(5*n0,2.5*n0,theta0));
% %                 D= -bwdist(~bw_out);
% %                 g2 = imimposemin(D,bw_cut);
% %                 bw_cut = imdilate(watershed(g2) == 0,strel('disk',1));% & (temp_sel);
% %                 bw_out = bw_out & (~ bw_cut);
...
%                 bw_out = imopen(bw_out,strel('disk',10));
                ...
                bw_out  = ellipse_fit(bw_out,2);   %%% refine the mask using elliptical shape
...
% %                 bw_out  = ellipse_fit(bw_out,25);   %%% refine the mask using elliptical shape
                ...
                if back_th > 0
                    ...
                    bw_refine0 = imfill(bw_out,'holes');
                end
                if r_close > 0
