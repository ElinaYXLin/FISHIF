function [nucleus_profile,cytoplasmic_profile,quanti_p,varargout] = protein_profile3D2_emmask(nucleus_DAPI_profile,max_image,EL_info,signal_channel,mask_stack,signal_stack,image_folder,N_cycle,resolution,resolutionz,flip_EL,varargin)
...
mask_stack=mask_stack.*double(em_mask1);
...
%     sg_prop = regionprops(double(imerode(logical(mask_stack(:,:,I_layer)), strel('disk',r_edge))).*double(mask_stack(:,:,I_layer)),double(signal_stack(:,:,I_layer)),'MeanIntensity','PixelValues');
    sg_prop = regionprops(double(imerode(logical(mask_stack(:,:,I_layer)), strel('disk',r_edge))).*double(mask_stack(:,:,I_layer)),double(DAPI_stack(:,:,I_layer)),'MeanIntensity');
    sg2_prop = regionprops(double(imerode(logical(mask_stack(:,:,I_layer)), strel('disk',r_edge))).*double(mask_stack(:,:,I_layer)),double(DAPI_stack(:,:,I_layer)).^2,'MeanIntensity');
    if I_layer == 1
