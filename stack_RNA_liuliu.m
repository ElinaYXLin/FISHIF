function varargout = stack_RNA_liuliu(varargin)
% STACK_RNA_LIULIU M-file for stack_RNA_liuliu.fig
%      STACK_RNA_LIULIU, by itself, creates a new STACK_RNA_LIULIU or raises the existing
%      singleton*.
%
%      H = STACK_RNA_LIULIU returns the handle to a new STACK_RNA_LIULIU or the handle to
%      the existing singleton*.
%
%      STACK_RNA_LIULIU('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in STACK_RNA_LIULIU.M with the given input arguments.
%
%      STACK_RNA_LIULIU('Property','Value',...) creates a new STACK_RNA_LIULIU or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before stack_RNA_liuliu_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to stack_RNA_liuliu_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help stack_RNA_liuliu

% Last Modified by GUIDE v2.5 12-Feb-2014 17:30:26

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @stack_RNA_liuliu_OpeningFcn, ...
                   'gui_OutputFcn',  @stack_RNA_liuliu_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before stack_RNA_liuliu is made visible.
function stack_RNA_liuliu_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to stack_RNA_liuliu (see VARARGIN)
...
% Choose default command line output for stack_RNA_liuliu
...
% end
% UIWAIT makes stack_RNA_liuliu wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = stack_RNA_liuliu_OutputFcn(hObject, eventdata, handles) 
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global zmin zmax imstack imstack0 immask dlayer dcontrast dcontrmin dfsize dimthresh dmask dpeak mstatus resolution0 L_ratio g vlayer vcontrast vcontrmin vfsize vimthresh vmask vpeak limsize limthresh limcontrast max_spot n0 xout0 n xout imname imfolder bk_label bk_use mask_out stack_th mask_stack
imstack = zeros(0);
imstack0 = zeros(0);
immask = false(0);
max_spot = zeros(0);
...
if size(num_list,2) >= 13
    zmin = num_list(I_file,13);
else
    zmin = [];
end
if size(num_list,2) >= 14
    zmax = num_list(I_file,14);
else
    zmax = [];
end
...
% imstack0 = imstack0(:,:,8:end);
% imstack  = imstack(:,:,8:end);
...
% % %     lim = [0:2:1000];
% % %     decrease_th = 0.9^((lim(2)-lim(1))/20);
% % %     [stack_th,t0,dt0] = th_find2(spmask1(imstack,1),lim,decrease_th,low_th);
end
...
%     max_spot = spfilter(max_spot,rxy,rxy,1);   %%% Fine filter to get rid of noise
    ...
    mask_out(:,:,[1:zmin-1,zmax+1:end]) = false;
    mask_out2D(:,:,[1:zmin-1,zmax+1:end]) = false;
...
temp = lim(I1(find(lim(I1) >= low_th,2))+2);
if isempty(temp)
    lim1 = low_th;
else
    lim1 = temp(end);
...
function [stack_th,t0,dt0] = th_find2(mask_out,lim,decrease_th,varargin)
global imstack
...
[~,I1] = findpeaks(conv(diff(conv(dt0,H(5,:)/sum(H(5,:)),'same')),H(5,:)/sum(H(5,:)),'same'));
[~,I2] = findpeaks(-conv(diff(conv(dt0,H(5,:)/sum(H(5,:)),'same')),H(5,:)/sum(H(5,:)),'same'));
lim1 = lim(I1(find(lim(I1) >= low_th,1))+3);
if isempty(lim1)
    lim1 = low_th;
end
lim2 = lim(I2(find(lim(I2) >= lim1,1))+2);
...
if strcmp(switch_reply,'Yes')
    stack_RNA_liuliu_OpeningFcn(hObject, eventdata, handles)
else
