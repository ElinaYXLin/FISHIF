function varargout = stack_RNA_new(varargin)
% STACK_RNA_NEW M-file for stack_RNA_new.fig
%      STACK_RNA_NEW, by itself, creates a new STACK_RNA_NEW or raises the existing
%      singleton*.
%
%      H = STACK_RNA_NEW returns the handle to a new STACK_RNA_NEW or the handle to
%      the existing singleton*.
%
%      STACK_RNA_NEW('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in STACK_RNA_NEW.M with the given input arguments.
%
%      STACK_RNA_NEW('Property','Value',...) creates a new STACK_RNA_NEW or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before stack_RNA_new_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to stack_RNA_new_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help stack_RNA_new

% Last Modified by GUIDE v2.5 02-May-2015 16:58:22

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @stack_RNA_new_OpeningFcn, ...
                   'gui_OutputFcn',  @stack_RNA_new_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before stack_RNA_new is made visible.
function stack_RNA_new_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to stack_RNA_new (see VARARGIN)
...
% Choose default command line output for stack_RNA_new
...
% end
% UIWAIT makes stack_RNA_new wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = stack_RNA_new_OutputFcn(hObject, eventdata, handles) 
...
if isempty(get(handles.ch_RNA,'Value'))
    RNA_channel = num_list(I_file,10); %%% RNA channel
    % RNA_channel = num_list(I_file,8);
else
    RNA_channel = str2num(get(handles.ch_RNA,'Value'));
...
if strcmp(switch_reply,'Yes')
    stack_RNA_new_OpeningFcn(hObject, eventdata, handles)
else
...
function ch_RNA_Callback(hObject, eventdata, handles)
% hObject    handle to ch_RNA (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of ch_RNA as text
%        str2double(get(hObject,'String')) returns contents of ch_RNA as a double
temp_ch = floor(str2num(get(handles.ch_RNA,'String')));

if temp_ch > 0
    switch_reply = questdlg(['Modify channel index to ',num2str(temp_ch),'?']);

    if strcmp(switch_reply,'Yes')
        set(handles.ch_RNA,'String',num2str(temp_ch));
    else
        set(handles.ch_RNA,'String','');
    end
else
    set(handles.ch_RNA,'String','');
end
...
% --- Executes during object creation, after setting all properties.
function ch_RNA_CreateFcn(hObject, eventdata, handles)
% hObject    handle to ch_RNA (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% hObject    handle to ch_name (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of ch_name as text
%        str2double(get(hObject,'String')) returns contents of ch_name as a double
global channel2_add

temp_name = get(handles.ch_name,'String');

switch_reply = questdlg(['Modify channel name to ',temp_name,'?']);

if strcmp(switch_reply,'Yes')
    if isempty(temp_name)
        channel2_add = '';
    else
        channel2_add = ['_',temp_name];
    end
    stack_RNA_new_OpeningFcn(hObject, eventdata, handles)
else
    if isempty(channel2_add)
        temp_name = '';
    else
        temp_name = channel2_add(2:end);
    end
    set(handles.ch_name,'String',temp_name);
end
