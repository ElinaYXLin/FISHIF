function nuclei_overlap_detect_specified(lf_name,sp_name,varargin)
%clear all
close all
tic
...
if isa(lf_name,'char') && strcmp(lf_name(end-3:end),'.xls')
    ...
    floder_new=cellfun(@(x) [pathname,x],folder_list(:,1),'UniformOutput',false);
    folder_list(:,1)=floder_new;
...
%     channel_name = eval(folder_list{list_I,5});
    ...
    for list_J = list_J_all%1:M1
        image_folder=[folder_list{list_I,1},'stacks\',sub_list{list_J,3}];
        time_num = size(dir([image_folder,['*stack01','*.tif']]),1);
        for time_index=1:time_num
            mask_name=[time_name,num2str(time_index,'%04u'),'mask.mat'];
            try
                load([folder_list{list_I,1},out_folder,sub_list{list_J,3},mask_name]);
            catch
                load([folder_list{list_I,1},out_folder,sub_list{list_J,3},'mask.mat']);
            end
            try
                load([folder_list{list_I,1},result_folder,sub_list{list_J,3}(1:end-1),'_time',num2str(time_index,'%04u'),mat_tail],'max_image','DAPI_channel');
            catch
    ...
    % %         save([folder_list{list_I,1},out_folder,sub_list{list_J,3},mask_name],'mask_stack','-v7.3')
            saveas(gcf,[folder_list{list_I,1},out_folder,sub_list{list_J,3},time_name,num2str(time_index,'%04u'),mask_imname])
            if time_num==1
    ...
    %% Detection result output: %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % %         save([folder_list{list_I,1},out_folder,sub_list{list_J,3},mask_name],'mask_stack','-v7.3')
            saveas(gcf,[folder_list{list_I,1},out_folder,sub_list{list_J,3},time_name,num2str(time_index,'%04u'),mask_ovname])
            if time_num==1
