...
for I_layer = 1:(size(raw3D,3)-1)
    ...
    temp_label2 = zeros(size(raw3D(:,:,1)),'uint16');
    bwlabel1 = uint16(bwlabel(raw3D(:,:,I_layer)));
    bwlabel2 = uint16(bwlabel(raw3D(:,:,I_layer+1)));
    ...
    if ~isempty(prop1)
        ...
        mask_label(:,:,I_layer+1) = (bwlabel2+max_label).*uint16(raw3D(:,:,I_layer+1));
        max_label = max(mask_label(:));
    end
%     figure; imshow(mask_label(:,:,1));
    escape_time=toc;
    waitbar(I_layer/(size(raw3D,3)-1),h_wait,['Progress',num2str(I_layer/(size(raw3D,3)-1),'%.2f'),char(13,10)',...
        'Escape time: ',num2str(escape_time/3600,'%.3f'),' hours',char(13,10)',...
        'Remaining time required: ',num2str(((size(raw3D,3)-1)-I_layer)*escape_time/3600/I_layer,'%.3f'),' hours'])
end
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
