function [x_shift,y_shift,rotate_angle]=im_rectify_v2(im1_path,im2_path,channel_num)
...
%% rotate check
AngleR=10;
RectM=round(0.8*L1_R1/2*cos(AngleR/180*pi));
RectN=round(RectM*0.25);
ScanRegion=zeros(L1_R1,L2_R1);
CenterOfImage=round([L1_R1,L2_R1]./2);
rotateCheckRegionY=CenterOfImage(1)-RectM:CenterOfImage(1)+RectM;
rotateCheckRegionX=CenterOfImage(2)-RectN:CenterOfImage(2)+RectN;

% ScanRegion(CenterOfImage(1)-RectM:CenterOfImage(1),CenterOfImage(2)-RectN:CenterOfImage(2)+RectN)=1;
% ScanRegion=logical(ScanRegion);
Im1Scan_2=im1_rectify(rotateCheckRegionY,rotateCheckRegionX);
rotateCheckMatrix=[-10:0.01:10];
ssim_r=[];
for r_i=1:length(rotateCheckMatrix)
    rotateAngle=rotateCheckMatrix(r_i);
    Im2R=imrotate(im2_rectify,rotateAngle,"crop");
    Im2Scan_2=Im2R(rotateCheckRegionY,rotateCheckRegionX);
    ssimval = ssim(Im1Scan_2,Im2Scan_2);
    ssim_r(r_i)=ssimval;
end
[~,MI]=max(ssim_r);
rotateAngleBest=rotateCheckMatrix(MI);
im2_rectify_check=imrotate(im2_rectify,rotateAngleBest,"crop");

figure;subplot(1,2,1);imshow(im1_rectify,[]);subplot(1,2,2);imshow(im2_rectify_check,[]);
saveas(gcf,[im2_path,'allstack\','rectify','.fig']);
close(gcf)
figure;imshowpair(im1_rectify,im2_rectify_check,'falsecolor');
saveas(gcf,[im2_path,'allstack\','rectify_pair','.fig']);
close(gcf)

rotate_angle=rotate_angle+rotateAngleBest;

figure;imshow(ScanRegion,[])
figure;imshow(Im2Scan_2,[])
figure;imshowpair(Im1Scan_2,Im2Scan_2,'falsecolor');
figure;subplot(1,2,1);imshow(Im1Scan_2,[]);subplot(1,2,2);imshow(Im2Scan_2,[]);
figure;imshowpair(Im1Scan_2,Im2Scan_2,'falsecolor');
