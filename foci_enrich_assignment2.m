...
figure(1);maximize(1)
figure(2);maximize(2)
pool_name = parpool(35);
%% Data analysis: %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for list_I = 1%:N1
    [sub_num, sub_list] = xlsread([folder_list{list_I,1},in_folder,input_name]);
    [M1,M2] = size(sub_list);
    
    for list_J = 1:2%:M1%eval(run_list{list_I})
...
%         image_folder = [folder_list{list_I,1},in_folder,sub_list{list_J,3}];
        ...
        load([result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),mat_tail],'nucleus_protein_profile','nucleus_protein_profile_ab','quanti_p','max_image','em_mask');
...
%         C_foci = C_foci(N00);
        ...
        warning('off')
        parfor jj = 1:size(foci_list,1)
            foci_circle_area_temp = zeros(length(xl),length(dz));
            for zz = 1:length(dz)
                        ...
                        if ~isempty(nuclei_bound)
                            ...
                            for kk = 1:length(xl)
                                if xl(kk) > 0
                                    ...
                                    foci_circle_area_temp(kk,zz) = area(intersect(Pnb,Pcircle))*resolution^2;
                                end
                            end
                        end
                    end
            ...
            foci_circle_area(jj,:,:) = foci_circle_area_temp;
        end
        warning('on')
        foci_circle_area_cell = cat(1,foci_circle_area_cell,{foci_circle_area,xl});
...
%         dxyz0 = cat(1,dxyz0,dxyz(:));
        ...
        Nu_en_cell = cat(1,Nu_en_cell,{Nuclei_foci2});
        ...
        Nu_RNA_cell = cat(1,Nu_RNA_cell,{Nuclei_foci});
