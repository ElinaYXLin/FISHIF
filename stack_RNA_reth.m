function stack_RNA_reth(file_list,single_switch,varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to stack_RNA_lx (see VARARGIN)
global imstack immask dlayer dcontrast dcontrmin dfsize dimthresh dmask dpeak mstatus resolution0 L_ratio g vlayer vcontrast vcontrmin vfsize vimthresh vmask vpeak limsize limthresh limcontrast bk_label sub_folder bk_name sg_name stack_th stack_th0 single_switch0 AP_switch0 thresh_set
...
function load_file
global imstack imstack0 immask dlayer dcontrast dcontrmin dfsize dimthresh dmask dpeak mstatus resolution0 L_ratio g vlayer vcontrast vcontrmin vfsize vimthresh vmask vpeak limsize limthresh limcontrast max_spot n0 xout0 n xout imname imfolder bk_label bk_use mask_out stack_th single_switch0 AP_switch0 mask_stack
imstack = zeros(0);
imstack0 = zeros(0);
immask = false(0);
max_spot = zeros(0);
...
rxy = 10;
mat_tail = '.mat';
mat_name = [imname,'_value0',mat_tail];
...
load([imfolder0,sub_folder,mat_name],'max_spot')
xx = round(max_spot(:,6)); xx(xx<1) = 1; xx(xx>size(imstack,1)) = size(imstack,1);
yy = round(max_spot(:,7)); yy(yy<1) = 1; yy(yy>size(imstack,2)) = size(imstack,2);
zz = round(max_spot(:,8)); zz(zz<1) = 1; zz(zz>size(imstack,3)) = size(imstack,3);
ind0 = sub2ind(size(imstack),xx,yy,zz);
Inten0 = imstack(ind0);
Itrue = Inten0*65535 >= stack_th;
max_spot = max_spot(Itrue,:);
...
if ~exist([imfolder0,sub_folder,mat_name2],'file')
    copyfile([imfolder0,sub_folder,mat_name],[imfolder0,sub_folder,mat_name2]);
end
