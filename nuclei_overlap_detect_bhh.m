function nuclei_overlap_detect_bhh(lf_name,varargin)
%clear all
close all
tic
...
%% mask overlap plot: %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        N_z=size(mask_stack,3);
        Diff=repmat(~diff_mask,1,1,N_z);
        mask_diff=double(Diff).*double(mask_stack);
        Iz=max(logical(mask_diff(:,:,:)),[],3);
        number=length(unique(mask_diff(:,:,:)));
        I1=split(sub_list{list_J,3},'/');
        imshow(Iz);
        imwrite(Iz,[folder_list{list_I,1},'masks2D/',char(I1(1,:)),'_',sprintf('%d',number),'.tif'],'tif');
