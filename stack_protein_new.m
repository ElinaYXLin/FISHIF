function varargout = stack_protein_new(varargin)
% STACK_PROTEIN_NEW M-file for stack_protein_new.fig
%      STACK_PROTEIN_NEW, by itself, creates a new STACK_PROTEIN_NEW or raises the existing
%      singleton*.
%
%      H = STACK_PROTEIN_NEW returns the handle to a new STACK_PROTEIN_NEW or the handle to
%      the existing singleton*.
%
%      STACK_PROTEIN_NEW('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in STACK_PROTEIN_NEW.M with the given input arguments.
%
%      STACK_PROTEIN_NEW('Property','Value',...) creates a new STACK_PROTEIN_NEW or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before stack_protein_new_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to stack_protein_new_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help stack_protein_new

% Last Modified by GUIDE v2.5 07-Mar-2012 17:29:18

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @stack_protein_new_OpeningFcn, ...
                   'gui_OutputFcn',  @stack_protein_new_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before stack_protein_new is made visible.
function stack_protein_new_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to stack_protein_new (see VARARGIN)
...
% Choose default command line output for stack_protein_new
...
% end
% UIWAIT makes stack_protein_new wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = stack_protein_new_OutputFcn(hObject, eventdata, handles) 
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global imstack imstack0 immask dlayer dcontrast dcontrmin dfsize dimthresh dmask dpeak mstatus resolution0 L_ratio g vlayer vcontrast vcontrmin vfsize vimthresh vmask vpeak limsize limthresh limcontrast max_spot n0 xout0 n xout imname imfolder bk_label bk_use mask_out stack_th immaskout
imstack = zeros(0);
imstack0 = zeros(0);
immask = false(0);
max_spot = zeros(0);
...
if mean(mean(mean(imstack0(ymin1:ymax1,xmin1:xmax1,:)))) >= mean(mean(mean(imstack0(ymin2:ymax2,xmin2:xmax2,:))))
    ...
    immaskout = repmat(~imdilate(max(logical(mask_stack(ymin1:ymax1,xmin1:xmax1,:)),[],3),strel('disk',5)),[1,1,size(mask_stack,3)]); 
else
    imstack0 = imstack0(ymin2:ymax2,xmin2:xmax2,:);
    imstack  = imstack(ymin2:ymax2,xmin2:xmax2,:);
    immaskout = repmat(~imdilate(max(logical(mask_stack(ymin2:ymax2,xmin2:xmax2,:)),[],3),strel('disk',5)),[1,1,size(mask_stack,3)]); 
end
...
%% Image output: %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
mask_out = spmask(imstack.*immaskout,1,stack_th);   %%% 3D local maximal mask generation (Primary filter)
...
[stack_th,t0,dt0] = th_find(spmask(imstack.*immaskout,1),lim,decrease_th);
...
if get(handles.presult_on,'Value') && exist([imfolder0,sub_folder,xls_name2],'file')
    ...
    [mask_out,mask_out2D] = spmask(imstack.*immaskout,1,stack_th);   %%% 3D local maximal mask generation (Primary filter)
...
function [stack_th,t0,dt0] = th_find(mask_out,lim,decrease_th)
global imstack

t0 = zeros(size(lim));
for I_th = 1:length(lim)
    t0(I_th) = nnz(mask_out & (imstack*65535 >=lim(I_th)));
end
dt0 = exp(diff(log(t0)));
dt0(end+1) = nan;
stack_th1 = lim(find(dt0 == max(dt0),1));

[~,I0] = min(dt0(t0 > 10));
I1 = find(dt0(I0:end) >= decrease_th,1)+I0-1;
stack_th2 = lim(I1);
