...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
out_title = {'#','Sample','Cycle','resolution','resolutionz','quanti_p','Cmax0'};
NI = 0;
out_txt = cell(0);
out_num = zeros(0);
...
for list_I = 1:2%N1
    ...
    for list_J = 1:M1%eval(run_list{list_I})
        ...
        load([result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),mat_tail],'quanti_p','resolution','resolution','resolutionz','nucleus_protein_profile_ab','nucleus_protein_profile');
        N_cycle = sub_num(list_J,13);
        index_true = nucleus_protein_profile_ab(:,4) > 1 & nucleus_protein_profile_ab(:,4) < 23;
        ind0 = nucleus_protein_profile_ab(:,1) < 0.5;
        quanti_p = polyfit(nucleus_protein_profile(index_true & ind0,2),nucleus_protein_profile(index_true & ind0,3),1);
        quanti_p(1) = quanti_p(1).*resolution.*resolution.*resolutionz.*6.02e8.*4.*pi.*sigma.^2.*2.*sqrt(pi).*sigmaz;
        Cmax0 = max(nucleus_protein_profile_ab(index_true,2));
        
        NI = NI+1;
        out_txt = cat(1,out_txt,{image_folder});
        out_num = cat(1,out_num,[N_cycle,resolution,resolutionz,quanti_p(1),Cmax0]);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%        
