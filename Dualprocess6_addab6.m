...
for list_I = 1:N1
    ...
    for list_J = 1:M1%eval(run_list{list_I})
        ...
        load([result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),mat_tail],'nucleus_RNA_profile','nucleus_protein_profile');
        Itrue = nucleus_protein_profile(:,4) > 1 & nucleus_protein_profile(:,4) < 23;
        
        TXnoise_plot(nucleus_RNA_profile(Itrue,:),N_cycle,image_folder,[101,102]);
