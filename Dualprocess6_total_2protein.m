...
bin_min2 = 0:1:100;   %%% bin_min for [Bcd]
bin_max2 = 3:1:103;   %%% bin_max for [Bcd]
...
% NbinE = 20;
% roverE = 0.6;
NbinE = 20;   %%% Equal population binning for enrichment
NmoveE = 250;
RNAmax = 150;
...
% ana_folder = 'Dualprocess6_result/02072016_16249-1-5M_Cad_Bcd_PBcd_original_08032017/';
% ana_folder = 'Dualprocess6_result/02072016_16249-1-5M_Cad_Bcd_PBcd_original_10032018/';
ana_folder = 'Dualprocess6_result/02072016_16249-1-5M_Cad_Bcd_PCad_original_10072018/';
...
xls_title3 = {'Name','Cycle','h','EL0','Cmax','Cmin','Cmax0'};
xls_title0 = {'Name','Cycle','#nuclei','Nu/Cyto','Cmax (nM)'};
% xls_title4 = {'Name','Cycle','k','c0'};
...
% % expf0 = fittype( @(a,b,c, x) a*exp(-x/b)+c);
% % expf0_line000 = 'protein_prefit = zeros(length(v0.savedSession.AllFitdevsAndConfigs),4);';
% % expf0_line00 = 'protein_prefit(tempI,:) = [tempI,temp0.a,temp0.b,temp0.c];';
% % expf0_line0 = 'beta0 = [max(ybin0),0.2,0];';
% % expf0_line1 = 'beta1 = [curve.a,curve.b,curve.c];';
% % expf0_line2 = 'curve.a = nan; curve.b = nan; curve.c = nan;';
% % expf0_line3 = 'curve.a = beta1(1); curve.b = beta1(2); curve.c = beta1(3);';
% % expf0_line4 = 'text(mean(xlim0),mean(ylim2b),[''h = '',num2str(beta1(1),''%5.2f''),char(10),''EL0 = '',num2str(beta1(2),''%5.2f''),char(10),''ymax = '',num2str(beta1(3),''%5.2f'')])';
expf0 = fittype( @(h,c,a,d,x) a*x.^h./(c^h+x.^h)+d);
expf0_line000 = 'protein_prefit = zeros(length(v0.savedSession.AllFitdevsAndConfigs),5);';
expf0_line00 = 'protein_prefit(tempI,:) = [tempI,temp0.h,temp0.c,temp0.a,temp0.d];';
expf0_line0 = 'beta0 = [5,0.4,max(ybin0),0];';
expf0_line1 = 'beta1 = [curve.h,curve.c,curve.a,curve.d];';
expf0_line2 = 'curve.h = nan; curve.c = nan; curve.a = nan; curve.d = nan;';
expf0_line3 = 'curve.h = beta1(1); curve.c = beta1(2); curve.a = beta1(3); curve.d = beta1(4);';
expf0_line4 = 'text(mean(xlim0),mean(ylim2b),[''h = '',num2str(beta1(1),''%5.2f''),char(10),''EL0 = '',num2str(beta1(2),''%5.2f''),char(10),''ymax = '',num2str(beta1(3),''%5.2f''),char(10),''ymin = '',num2str(beta1(4),''%5.2f'')])';
...
%             image_folder = [folder_list{list_I,1},in_folder,sub_list{list_J,3}(1:end-1),old_add,'/'];
            ...
            p_protein_all = [p_protein_all;p_protein];
...
%                         [beta1,r,~,~,~] = nlinfit(xbin(I_fit),ybin(I_fit),Hill1,beta0);
                        curve = fit(xbin0',ybin0', expf0, 'StartPoint',beta0,'Lower',zeros(size(beta0)));
                        eval(expf0_line1);
                        H_I = beta1;
                    catch err
                        beta1 = nan(size(beta0));
...
% % % % % %                     end
                    ...
                    curve = fit(xbin0',ybin0', expf0, 'StartPoint',beta1,'Lower',zeros(beta1));
                    eval(expf0_line3);
                end
...
%                 beta_all3 = [beta_all3;beta1];
                ...
                hold on
                errorbarxy(xbin,ybin,xerr,yerr,xerr,yerr,'k','k','.','DisplayName','Binned data');
                plot([xlim0(1):xlim_bin:xlim0(2)],feval(curve,[xlim0(1):xlim_bin:xlim0(2)]),'r-','DisplayName','Exp fit');
...
%             ha1 = subplot(sub_ij(1),sub_ij(2),i00);
            ha1 = subaxis(sub_ij(1),sub_ij(2),i00, 'Spacing', 0, 'PaddingRight',0.03, 'PaddingLeft',0.05, 'PaddingTop',0.03, 'PaddingBottom',0.03, 'MarginLeft', 0.01, 'MarginRight', 0.001, 'MarginTop', 0.015, 'MarginBottom', 0.025);
                [xbin,ybin,xerr,yerr,N_in] = equal_bin(nucleus_protein_profile_ab(Izr,2),nucleus_RNA_profile(Izr,3),Nbin,rover,1);
...
%                         [beta1,r,~,~,~] = nlinfit(xbin(I_fit),ybin(I_fit),Hill1,beta0);
                    ...
                    curve = fit(xbin(I_fit)',ybin(I_fit)', g0, 'StartPoint',beta1,'Upper',5*max(beta1,max(beta0,zeros(size(beta0)))),'Lower',5*min(beta1,min(beta0,zeros(size(beta0)))));
...
%             ha1 = subplot(sub_ij(1),sub_ij(2),i00);
            ha1 = subaxis(sub_ij(1),sub_ij(2),i00, 'Spacing', 0, 'PaddingRight',0.03, 'PaddingLeft',0.05, 'PaddingTop',0.03, 'PaddingBottom',0.03, 'MarginLeft', 0.01, 'MarginRight', 0.001, 'MarginTop', 0.015, 'MarginBottom', 0.025);
                [xbin,ybin,xerr,yerr,N_in] = equal_bin(nucleus_protein_profile_ab(Izr,2),nucleus_signal2_profile(Izr,3),Nbin,rover,1);
                I00 = ~isnan(xbin) & ~isnan(ybin) & N_in >= 5;
                xbin = xbin(I00);
                ybin = ybin(I00);
                xerr = xerr(I00);
...
%             ha1 = subplot(sub_ij(1),sub_ij(2),i00);
            ha1 = subaxis(sub_ij(1),sub_ij(2),i00, 'Spacing', 0, 'PaddingRight',0.03, 'PaddingLeft',0.05, 'PaddingTop',0.03, 'PaddingBottom',0.03, 'MarginLeft', 0.01, 'MarginRight', 0.001, 'MarginTop', 0.015, 'MarginBottom', 0.025);
                [xbin,ybin,xerr,yerr,N_in] = equal_bin(nucleus_protein_profile_ab(Izr,2),nucleus_RNA_profile(Izr,4),Nbin,rover,1);
                I00 = ~isnan(xbin) & ~isnan(ybin) & N_in >= 5;
                xbin = xbin(I00);
                ybin = ybin(I00);
                xerr = xerr(I00);
...
%                         [beta2,r,~,~,~] = nlinfit(xbin(I_fit),ybin(I_fit),Hill1,beta0);
                    ...
                    curve = fit(xbin(I_fit)',ybin(I_fit)', g0, 'StartPoint',beta2,'Upper',5*max(beta2,max(beta0,zeros(size(beta0)))),'Lower',5*min(beta2,min(beta0,zeros(size(beta0)))));
...
%             ha1 = subplot(sub_ij(1),sub_ij(2),i00);
            ha1 = subaxis(sub_ij(1),sub_ij(2),i00, 'Spacing', 0, 'PaddingRight',0.03, 'PaddingLeft',0.05, 'PaddingTop',0.03, 'PaddingBottom',0.03, 'MarginLeft', 0.01, 'MarginRight', 0.001, 'MarginTop', 0.015, 'MarginBottom', 0.025);
                [xbin,ybin,xerr,yerr,N_in] = equal_bin(nucleus_protein_profile_ab(Izr,2),nucleus_signal2_profile(Izr,4),Nbin,rover,1);
                I00 = ~isnan(xbin) & ~isnan(ybin) & N_in >= 5;
                xbin = xbin(I00);
                ybin = ybin(I00);
                xerr = xerr(I00);
...
%% Data output: %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    out_ex0 = cat(1,xls_title5,cat(2,name_all,num2cell(cycle_all),num2cell(p_protein_all)));
    xlswrite([ana_folder,embryo_name{iem},'/',embryo_name{iem},nu_add,protein_add,fluc_add,D3_add,output_tail],out_ex0);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%        
