%% spatiotemporal Bcd Hb Kr in nc11 and fit (fit parameter result:co, saved in "fit")
clear 
close all
[num,txt,raw]=xlsread('X:/baohuihan/FISHIF-Time/Confocal/Fit.xlsx','cycle11');%% input:data information integrated in Excel
...
for j=2:length(txt(:,1))
    ...
    nucleus_bin = 1:0.25:7;
    average_radius = 0.5;
 bin_max = min(nucleus_bin+average_radius,7);
...
Hb=nuHb(5:15,:);
t=timer;
save([bhh_path,'fit/Bcd_11_all.mat'],'nu2');
save([bhh_path,'fit/Hb_11_all.mat'],'nuHb');
save([bhh_path,'fit/R1_11_all.mat'],'fi0');

save([bhh_path,'fit/Bcd_11.mat'],'Bcd');
save([bhh_path,'fit/Hb_11.mat'],'Hb');
save([bhh_path,'fit/R1_11.mat'],'R1');
save([bhh_path,'fit/t_11.mat'],'t');
%%
Kr=R1;
%Kr=R1;
co0=[0 20 2 8 5 7 1 t(1) 6];
lb=[0 1 1 1 1 1 0.3 3 5];
ub=[10 30 20 20 10 10 5 6 7];
[co,resnorm,residual,exitflag,output,lambda,jacobian] = lsqcurvefit(@myfun_cycle11,co0,EL,Kr,lb,ub);
conf=nlparci(co,residual,'jacobian',jacobian,"Alpha",0.9);%90% confidence
save([bhh_path,'fit/co_11.mat'],'co');
%%   Å·À­·¨
