...
for list_I = 1:N1
    ...
    for list_J = 1:M1
        ...
        if max(max(bwlabel(seg_bw))) >= 20
            [mask_stack,signal_stack,GFP_stack] = mask3D(seg_bw,protein_channel,DAPI_channel,GFP_channel,image_folder);
            ...
            [nucleus_GFP_profile,cytoplasmic_GFP_profile] = protein_profile3(seg_bw,cyto_bw,max_image,GFP_channel,mask_stack,GFP_stack,image_folder,N_cycle,resolution,'GFP',22,24);
        else
            nucleus_protein_profile = [0,0];
            cytoplasmic_protein_profile = [0,0];
            nucleus_GFP_profile = [0,0];
            cytoplasmic_GFP_profile = [0,0];
            figure(2)
            figure(14)
            figure(22)
            figure(24)
...
%%% Output: %%%============================================================
        ...
        save([result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),mat_tail],'em_mask','image_folder','image_type','WGA_channel','DAPI_channel','protein_channel','GFP_channel','resolution','seg_bw','cyto_bw','max_image','nucleus_protein_profile','cytoplasmic_protein_profile','nucleus_GFP_profile','cytoplasmic_GFP_profile','N_cycle','WGA_th0','DAPI_th0');
