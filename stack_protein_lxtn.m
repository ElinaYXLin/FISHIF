function stack_protein_lxtn(file_list,single_switch)
...
if single_switch0
    sub_folder = 'Histogram_protein_An/';
else
    sub_folder = 'Histogram_protein_Pn/';
end
...
load([imfolder(1:find((imfolder(1:end-1) == '/') | (imfolder(1:end-1) == '\'),1,'last')),'masks/',imname,'/mask.mat']);
    ...
    if xor(~single_switch0,mean(mean(mean(imstack0(ymin1:ymax1,xmin1:xmax1,:)))) >= mean(mean(mean(imstack0(ymin2:ymax2,xmin2:xmax2,:)))))
...
% else
    ...
    embryo_mask = logical(mask_stack);
