...
for list_I = 6:N1
    ...
    for list_J = 1:M1%eval(run_list{list_I})
        ...
        [nucleus_protein_profile,cytoplasmic_protein_profile,quanti_p,nucleus_protein_ab,flip_EL] = protein_profile3D(max_image,protein_channel,mask_stack,signal_stack,image_folder,N_cycle,resolution,resolutionz,[],[],[],[],[],[],flip_axis);

        [nucleus_RNA_profile,foci_RNA_profile,cytoplasmic_RNA_profile] = RNA_profile3D(mask_stack,cyto_bw,foci_bw3D,max_image00,SS,RNA_stack,resolution,image_folder,N_cycle,[],flip_EL);   %%% Input the foci area matrix from modified_foci.m if gaussian fitting method is used
...
%         signal_stack = pro_temp(mask_stack+1);
        ...
        [nucleus_signal2_profile,foci_signal2_profile,cytoplasmic_signal2_profile] = RNA_profile3D(mask_stack,cyto_bw,foci_bw3D2,max_image002,SS2,signal2_stack,resolution,image_folder,N_cycle,[45,46,47,48,49,411],flip_EL);   %%% Input the foci area matrix from modified_foci.m if gaussian fitting method is used
        ...
        dual_profile(nucleus_protein_profile_ab,nucleus_RNA_profile,foci_RNA_profile,image_folder,N_cycle,z_size);
