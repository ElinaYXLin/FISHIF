max_image0 = double(max_image(:,:,3))/double(max(max(max_image(:,:,3))));
H = -fspecial('log',15,0.5/(resolution/0.13));
fit_radius = 10;
test_threshold = [0:0.001:0.3];
Ng = zeros(size(test_threshold));
Tg = zeros(size(test_threshold));
Ng_th = 10000;
...
title('Please check the threshold point (if not correct, click on the point you want)')
[x,y,button] = ginput(1);
if button == 1
    g_threshold0 = x;
end
...
g_prop = regionprops(bw_g,seg_bw,'Area','MeanIntensity','MaxIntensity','Eccentricity');
