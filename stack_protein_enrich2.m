function varargout = stack_protein_enrich2(varargin)
% STACK_PROTEIN_ENRICH2 M-file for stack_protein_enrich2.fig
%      STACK_PROTEIN_ENRICH2, by itself, creates a new STACK_PROTEIN_ENRICH2 or raises the existing
%      singleton*.
%
%      H = STACK_PROTEIN_ENRICH2 returns the handle to a new STACK_PROTEIN_ENRICH2 or the handle to
%      the existing singleton*.
%
%      STACK_PROTEIN_ENRICH2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in STACK_PROTEIN_ENRICH2.M with the given input arguments.
%
%      STACK_PROTEIN_ENRICH2('Property','Value',...) creates a new STACK_PROTEIN_ENRICH2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before stack_protein_enrich2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to stack_protein_enrich2_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help stack_protein_enrich2

% Last Modified by GUIDE v2.5 18-Nov-2018 13:21:28

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @stack_protein_enrich2_OpeningFcn, ...
                   'gui_OutputFcn',  @stack_protein_enrich2_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before stack_protein_enrich2 is made visible.
function stack_protein_enrich2_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to stack_protein_enrich2 (see VARARGIN)
...
% Choose default command line output for stack_protein_enrich2
...
% end
% UIWAIT makes stack_protein_enrich2 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = stack_protein_enrich2_OutputFcn(hObject, eventdata, handles) 
...
for I_layer = 1:immax
    ...
    imstack0(:,:,I_layer) = imstack(:,:,I_layer);
...
if strcmp(switch_reply,'Yes')
    stack_protein_enrich2_OpeningFcn(hObject, eventdata, handles)
else
