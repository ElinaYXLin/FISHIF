function Dual2process7(run_marker,run_list,run_list2)
% % clear all
% % close all
...
folder_list = folder_list(strcmpi(run_marker,folder_list(:,6)),:);
...
for list_I = run_list%1:N1
    ...
    for list_J = run_list2%1:M1%eval(run_list{list_I})
...
%         N_cycle = round(log2(max(mask_stack(:))))+2;   %%% Calculate the nuclear cycle number
        ...
        protein2_color = all_color{signal2_channel};
        protein_protein2_mismatch = mismatch_matrix{strcmp(protein_color,mismatch_matrix(1,:)),strcmp(protein2_color,mismatch_matrix(:,1))};
        protein_protein2_xymismatch = {eval([protein_color,'_',protein2_color]),eval([protein_color,'_',protein2_color,'_con']),eval([protein_color,'_',protein2_color,'_x0']),Nbin,Mdim,resolution,resolution_mismatch};

        [signal_stack,RNA_stack,signal2_stack,DAPI_stack] = stack3D(imclearborder(seg_bw),protein_channel,DAPI_channel,RNA_channel,image_folder,protein_RNA_mismatch,signal2_channel,protein_protein2_mismatch);   %%% load 3D image stacks
...
%         [nucleus_RNA_profile,foci_RNA_profile,cytoplasmic_RNA_profile] = RNA_profile3D(mask_stack,nucleus_DAPI_profile,cyto_bw,foci_bw3D,max_image00,EL_info,SS,RNA_stack,resolution,image_folder,N_cycle,[],flip_EL);   %%% Input the foci area matrix from modified_foci.m if gaussian fitting method is used
        ...
        [nucleus_protein2_profile,cytoplasmic_protein2_profile,quanti_p2,nucleus_protein2_ab,flip_EL] = protein_profile3D(nucleus_DAPI_profile,max_image,EL_info,signal2_channel,mask_stack,signal2_stack,image_folder,N_cycle,resolution,resolutionz,'Protein',22,24,52,53,[1.8,1.0],flip_axis);
        nucleus_protein2_profile_ab = [nucleus_protein2_profile(:,1),nucleus_protein2_ab,nucleus_protein2_profile(:,4),nucleus_protein2_profile(:,4)];
        signal2_stack = (signal2_stack+quanti_p2(2))/quanti_p2(1);
        nucleus_protein2_profile = [nucleus_protein2_profile,nucleus_DAPI_profile];
        ...
        [foci_data2,fake_data2,h2,t_absolute2,r_size2] = dual_local_local3D3(foci_bw00,max_image_list,mask_stack,DNA_mask,signal2_stack,RNA_stack,nucleus_protein2_profile_ab,image_folder,N_cycle,resolution,resolutionz,[],[473:477,480,481],[1.8,1.8,1.0],{foci_data,fake_data,r_size,protein_protein2_xymismatch});
        clear mask_stack signal_stack signal2_stack RNA_stack
        
        %% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

        dual_profile(nucleus_protein_profile_ab,nucleus_RNA_profile,foci_RNA_profile,image_folder,N_cycle,z_size,{'M','#'});
        dual_profile(nucleus_protein2_profile_ab,nucleus_RNA_profile,foci_RNA_profile,image_folder,N_cycle,z_size,{'M','#'},[32,35,33]);
        dual2_profile(nucleus_protein_profile_ab,nucleus_protein2_profile_ab,nucleus_RNA_profile,foci_RNA_profile,image_folder,N_cycle,z_size,{protein_add(2:end),signal2_add(2:end)},{'M','#'})
...
%%% Output: %%%============================================================
        ...
        saveas(22,[result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),signal2_add,D3_add,figure_tail]);
        saveas(24,[result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),signal2_add,fate_add,D3_add,figure_tail]);
        saveas(52,[result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),signal2_add,fluc_add,D3_add,figure_tail]);
        saveas(53,[result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),signal2_add,bino_add,D3_add,figure_tail]);
        ...
        saveas(32,[result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),nu_add,reg_add,D3_add,figure_tail]);
        saveas(33,[result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),foci_add,reg_add,D3_add,figure_tail]);
        saveas(35,[result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),nu_add,foci_add,reg_add,D3_add,figure_tail]);

        saveas(36,[result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),nu_add,reg2_add,D3_add,figure_tail]);
        saveas(38,[result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),foci_add,reg2_add,D3_add,figure_tail]);
        saveas(37,[result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),nu_add,foci_add,reg2_add,D3_add,figure_tail]);
        ...
        saveas(473,[result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),signal2_add,local_add,foci_add,D3_add,figure_tail]);
        saveas(474,[result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),signal2_add,local_add,hist_add,D3_add,figure_tail]);
        saveas(475,[result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),signal2_add,local_add,RNA_add,D3_add,figure_tail]);
        saveas(476,[result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),signal2_add,local_add,abs_add,foci_add,D3_add,figure_tail]);
        saveas(477,[result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),signal2_add,local_add,abs_add,hist_add,D3_add,figure_tail]);
        saveas(480,[result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),signal2_add,local_add,rad_add,D3_add,figure_tail]);
        saveas(481,[result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),signal2_add,local_add,abs_add,rad_add,D3_add,figure_tail]);
        ...
        save([result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),mat_tail],'image_folder','image_type','WGA_channel','DAPI_channel','RNA_channel','protein_channel','resolution','seg_bw','cyto_bw','max_image','foci_bw2D','nucleus_RNA_profile','foci_RNA_profile','cytoplasmic_RNA_profile','nucleus_protein_profile','cytoplasmic_protein_profile','N_cycle','foci_data','fake_data','h','t_absolute','r_size','foci_data2','fake_data2','h2','t_absolute2','r_size2','resolutionz','quanti_p','nucleus_protein_profile_ab','nucleus_protein2_profile','cytoplasmic_protein2_profile','quanti_p2','nucleus_protein2_profile_ab','-append');
        ...
        if ~isempty(nucleus_protein2_profile)
           xlswrite([result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),signal2_add,nu_add,output_tail],nucleus_protein2_profile);
        end
        if ~isempty(cytoplasmic_protein2_profile)
           xlswrite([result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),signal2_add,cyto_add,output_tail],cytoplasmic_protein2_profile);
        end
        ...
        clear ('seg_bw','cyto_bw','max_image','foci_bw','nucleus_RNA_profile','foci_RNA_profile','cytoplasmic_RNA_profile','nucleus_protein_profile','cytoplasmic_protein_profile','quanti_p','nucleus_protein_profile_ab','nucleus_protein2_profile','cytoplasmic_protein2_profile','quanti_p2','nucleus_protein2_profile_ab','N_cycle','foci_data','fake_data','h','t_absolute','r_size','foci_data2','fake_data2','h2','t_absolute2','r_size2');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%        
