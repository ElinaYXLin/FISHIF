...
%% segmentation test and mean nuclear size measurement: %%%%%%%%%%%%%%%%%%%
        ...
        im_max = max(fil_im(:));
        im_temp = fil_im;
        bw_im = gpuArray(false(size(fil_im)));

        for I = 1:(I_max-1)
            th_I = I/I_max*double(im_max)/65535;
            bw_temp = imfill(im_temp > th_I,'holes');
            bw_prop = regionprops(gather(bw_temp),'Area','Perimeter');%,'ConvexArea');
...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%        
        ...
        raw3D = gpuArray(false([size_im([1:2]),length(imlist)])); %%% Raw 3D mask
        th_3D = gpuArray(zeros([size_im([1:2]),length(imlist)])); %%% Raw 3D threshold
        fil3D = gpuArray(zeros([size_im([1:2]),length(imlist)])); %%% Smoothed image
...
% % pid = parpool(npar);
        for image_I = 10:14%1:length(imlist)
            raw_im = imread([folder_list{list_I,1},in_folder,sub_list{list_J,3},imlist(image_I).name]);
            if scale0 < 1
                raw_im = imresize(raw_im(1:end-n1,1:end-n2,:),scale0);
            end
...
%             I_blur =  adapthisteq(I_blur); %Step 1: enhances the contrast of the grayscale image   
            ...
            outims = outE - outI;  
            W = watershed(gather(max(outims(:))-outims));

            outims(outims < 0) = 0;
            outims = outims/max(outims(:));

            bw_diff = outims > 1.2*graythresh(gather(outims)); 
...
%% Circular segmentation: %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            H = fspecial('disk',imblur); % Filter Kernel  
            fil_im = imfilter(input_im,H,'same','conv');
            th_2D = gpuArray(zeros(size_im([1:2]))); %%% Raw 2D threshold
            im_max = max(fil_im(:));
            im_temp = fil_im;
            bw_im = gpuArray(false(size(fil_im)));
%%% Cicle search
...
%                 bw_temp = imopen(imfill(im2bw(im_temp,th_I),'holes'),strel('disk',10));
                bw_prop = regionprops(gather(bw_temp),'Area','Perimeter');
                bw_area = [bw_prop.Area];
                bw_perim = [bw_prop.Perimeter];
...
%% Supplement segmentation based on center recognition: %%%%%%%%%%%%%%%%%%%
            ...
            if ~isempty(bw_props)
                ...
                for I_center = 1:size(bw_center0,1)
                    ...
                    bw_th = imfill(bwareaclose(im_temp > th_estimate,merge_th),'holes');
...
%         mask_label = label3D(raw3D);   %%% mask tiling
        mask_stack = label3D_GPU(raw3D);   %%% mask tiling
...
%% Resign the label #: %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if scale0 < 1
            mask_stack = imresize(mask_stack,1/scale0);
