...
I_plot = 1:34;%[1:2,4:7,10:14,16,20];%[2:26,29:30,32];%[1:2,4:7,10:14,16,20];
I_cycle = 13;
...
% %     [n0,r0] = hist(dxy0,x0);
% %     nr = n0./(xr.^2-xl.^2)/pi;
% %         [na,r0] = hist(dxy0(EL_RNA0 < 0.4),x0);
% % % %     [na,r0] = hist(dxy0(C_RNA0(:,IC) > 10e-9),x0);
% % % %     nar = na./(xr.^2-xl.^2)/pi;
    id0 = C_RNA0(:,IC) > 10e-9;
    xdata = dxy0(id0);
    ydata0 = en_circle_diff(id0,:,:);
    [~,ybin,xind] = histw(xdata,ones(size(xdata)),xl,xr);
    xind(xind == 0) = size(ydata0,2);
    [~,Nz] = ismember(dz_en_RNA(id0),dz);
    id1 = sub2ind(size(ydata0),(1:length(xdata))',xind,Nz);
    ydata = 1./ydata0(id1)/sum(ybin);
    [r0,nar,~] = histw(xdata,ydata,xl,xr);
    pa = polyfit(r0(r0 >= rout_min & r0 <= rout_max),nar(r0 >= rout_min & r0 <= rout_max),1);
    
% %         [np,r0] = hist(dxy0(EL_RNA0 >= 0.7),x0);
% % % %     [np,r0] = hist(dxy0(C_RNA0(:,IC) < 5e-9),x0);
% % % %     npr = np./(xr.^2-xl.^2)/pi;
    id0 = C_RNA0(:,IC) < 5e-9;
    xdata = dxy0(id0);
    ydata0 = en_circle_diff(id0,:,:);
    ...
    id1 = sub2ind(size(ydata0),(1:length(xdata))',xind,Nz);
    ydata = 1./ydata0(id1)/sum(ybin);
    [r0,npr,~] = histw(xdata,ydata,xl,xr);
    pp = polyfit(r0(r0 >= rout_min & r0 <= rout_max),npr(r0 >= rout_min & r0 <= rout_max),1);
% %         plot(r0,nr,'k','DisplayName','All');
% %         hold on
    plot(r0,nar,'r','DisplayName','Anterior')
    hold on
    plot(r0,polyval(pa,r0),'r-.','DisplayName',['Anterior fit (k = ',num2str(pa(1)),')'])
    plot(r0,npr,'b','DisplayName','Posterior');
    plot(r0,polyval(pp,r0),'b-.','DisplayName',['Posterior fit (k = ',num2str(pp(1)),')'])
    xlim([0,xl(end)-1])
    xlabel('r (um)')
    ylabel('%/um^2')
...
subplot(2,3,4)
    [nH,r0] = hist(dxy0(I_RNA0 > 7),x0);
    nHr = nH./(xr.^2-xl.^2)/pi;
    pH = polyfit(r0(r0 >= rout_min & r0 <= rout_max),nHr(r0 >= rout_min & r0 <= rout_max)/sum(nH),1);
    [nL,r0] = hist(dxy0(I_RNA0 <= 2),x0);
    nLr = nL./(xr.^2-xl.^2)/pi;
    pL = polyfit(r0(r0 >= rout_min & r0 <= rout_max),nLr(r0 >= rout_min & r0 <= rout_max)/sum(nL),1);
% %         plot(r0,nr,'k','DisplayName','All');
% %         hold on
    plot(r0,nHr/sum(nH),'r','DisplayName','Active')
    hold on
    plot(r0,polyval(pH,r0),'r-.','DisplayName',['Active fit (k = ',num2str(pH(1)),')'])
    plot(r0,nLr/sum(nL),'b','DisplayName','Inactive');
    plot(r0,polyval(pL,r0),'b-.','DisplayName',['Inactive fit (k = ',num2str(pL(1)),')'])
    xlim([0,xl(end)-1])
    xlabel('r (um)')
    ylabel('%/um^2')
...
subplot(2,3,2)
    ...
    xr = xl+dx;
    x0 = xl+dx/2;
    [~,en_a,~,~] = equal_dist(dxy0(I_RNA0 > 7),I_en0(I_RNA0 > 7),xl,xr);
    [~,en_p,~,~] = equal_dist(dxy0(I_RNA0 <= 2),I_en0(I_RNA0 <= 2),xl,xr);
...
subplot(2,3,5)
    ...
    xr = xl+dx;
    x0 = xl+dx/2;
    [~,en_a,~,~] = equal_dist(dxy0(C_RNA0(:,IC) > 10e-9),I_en0(C_RNA0(:,IC) > 10e-9),xl,xr);
    [~,en_p,~,~] = equal_dist(dxy0(C_RNA0(:,IC) < 5e-9),I_en0(C_RNA0(:,IC) < 5e-9),xl,xr);
...
subplot(2,3,3)
    ...
    xl = x0;
    [n0,r0] = hist(N_en1,x0);
    I000 = EL_RNA1 < 0.7 & EL_RNA1 >= 0.05 & C_RNA1(:,IC) > 10e-9 & C_RNA1(:,IC) <= 15e-9;
    [na,r0] = hist(N_en1(I000),x0);
    I000 = EL_RNA1 > 0.7 & C_RNA1(:,IC) < 5e-9 & C_RNA1(:,IC) >= 1e-9;
    [np,r0] = hist(N_en1(I000),x0);
...
for ii = 1:length(EL_name)
    I000 = EL_RNA0 >= EL_range{ii}(1) & EL_RNA0 <= EL_range{ii}(2) & C_RNA0(:,IC) >= C_range{ii}(1) & C_RNA0(:,IC) <= C_range{ii}(2);
...
%     N111 = N_en1 == 1 & EL_RNA1 >= EL_range{ii}(1) & EL_RNA1 <= EL_range{ii}(2) & C_RNA1(:,IC) >= C_range{ii}(1) & C_RNA1(:,IC) <= C_range{ii}(2);
    Sa = pi*r_th^2;
    Sp = pi*(rout_max^2-rout_min^2);
    nf = hist0(I_en0(I000 & dxy0 > rout_min & dxy0 <= rout_max),xl,xr,true);
    Pfake = nf/nnz(I111)*Sa/Sp;
    ffake = nf/sum(nf);
    kf = sum(nf)/Sp/nnz(I111)*r_ad;
    
    for jj = 1:length(N_range0)
        I000 = N_en0 == N_range0(jj) & EL_RNA0 >= EL_range{ii}(1) & EL_RNA0 <= EL_range{ii}(2) & C_RNA0(:,IC) >= C_range{ii}(1) & C_RNA0(:,IC) <= C_range{ii}(2);
        ...
        if jj == 1
            ...
            Pob{ii,jj} = zeros(size(xl));
            PNreal{ii,jj} = PNob{ii,jj}/exp(-kf*Sa);
            Preal{ii,jj} = zeros(size(xl));
            freal{ii,jj} = zeros(size(xl));
        else
            nob = hist0(I_en0(I000 & dxy0 <= r_th),xl,xr,true);
            PNob{ii,jj} = nnz(N111)/nnz(I111);
            ...
            for kk = 1:jj-1
                PNreal{ii,jj} = PNreal{ii,jj}-PNreal{ii,kk}*exp(-kf*Sa)*(kf*Sa)^(N_range0(jj)-N_range0(kk))/factorial(N_range0(jj)-N_range0(kk));
                Preal{ii,jj} = Preal{ii,jj}-(N_range0(kk)*freal{ii,kk}+(N_range0(jj)-N_range0(kk))*ffake)*PNreal{ii,kk}*exp(-kf*Sa)*(kf*Sa)^(N_range0(jj)-N_range0(kk))/factorial(N_range0(jj)-N_range0(kk));
            end
            PNreal{ii,jj} = PNreal{ii,jj}/exp(-kf*Sa);
            Preal{ii,jj} = Preal{ii,jj}/exp(-kf*Sa);
...
% % %     
% % % figure 
% % % set(gcf,'Name',[embryo_name,': Single-spot model'])
% % % for ii = 1:length(EL_name)
% % %     I000 = EL_RNA0 >= EL_range{ii}(1) & EL_RNA0 <= EL_range{ii}(2) & C_RNA0(:,IC) >= C_range{ii}(1) & C_RNA0(:,IC) <= C_range{ii}(2);
% % %     I111 = EL_RNA1 >= EL_range{ii}(1) & EL_RNA1 <= EL_range{ii}(2) & C_RNA1(:,IC) >= C_range{ii}(1) & C_RNA1(:,IC) <= C_range{ii}(2);
% % % %     N111 = N_en1 == 1 & EL_RNA1 >= EL_range{ii}(1) & EL_RNA1 <= EL_range{ii}(2) & C_RNA1(:,IC) >= C_range{ii}(1) & C_RNA1(:,IC) <= C_range{ii}(2);
% % %     Sa = pi*r_th^2;
% % %     Sp = pi*(rout_max^2-rout_min^2);
% % %     nf = hist0(I_en0(I000 & dxy0 > rout_min & dxy0 <= rout_max)./r11(I000 & dxy0 > rout_min & dxy0 <= rout_max),xl,xr,true);
% % %     Pfake = nf/nnz(I111)*Sa/Sp;
% % %     ffake = nf/sum(nf);
% % %     kf = sum(nf)/Sp/nnz(I111)*r_ad;
% % %     for jj = 1:length(N_range0)
...
% % %             freal{ii,jj} = zeros(size(xl));
% % %         else
% % %             nob = hist0(I_en0(I000 & dxy0 <= r_th)./r11(I000 & dxy0 <= r_th),xl,xr,true);
% % %             PNob{ii,jj} = nnz(N111)/nnz(I111);
% % %             Pob{ii,jj} = nob/nnz(I111);
% % %             
% % %             PNreal{ii,jj} = (PNob{ii,jj}-PNreal{ii,1}*exp(-kf*Sa)*(kf*Sa)^N_range0(jj)/factorial(N_range0(jj)))/exp(-kf*Sa)/(kf*Sa)^(N_range0(jj)-1)*factorial(N_range0(jj)-1);
% % %             Preal{ii,jj} = (Pob{ii,jj}-N_range0(jj)*ffake*PNreal{ii,1}*exp(-kf*Sa)*(kf*Sa)^N_range0(jj)/factorial(N_range0(jj)))/exp(-kf*Sa)/(kf*Sa)^(N_range0(jj)-1)*factorial(N_range0(jj)-1)-(N_range0(jj)-1)*ffake*PNreal{ii,jj};
% % %             freal{ii,jj} = Preal{ii,jj}/PNreal{ii,jj};
% % %         end
% % % 
% % %         jj0 = find(N_range0(jj) == N_range);
...
% % % Pob = cell(length(EL_name),length(N_range0));
% % % PNreal = cell(length(EL_name),length(N_range0));
Preal = zeros(length(C_min),length(xl));
...
for ii = 1:length(C_min)
    I000 = EL_RNA0 >= EL_range(1) & EL_RNA0 <= EL_range(2) & C_RNA0(:,IC) >= C_min(ii) & C_RNA0(:,IC) <= C_max(ii);
...
%     N111 = N_en1 == 1 & EL_RNA1 >= EL_range{ii}(1) & EL_RNA1 <= EL_range{ii}(2) & C_RNA1(:,IC) >= C_range{ii}(1) & C_RNA1(:,IC) <= C_range{ii}(2);
    ...
    xxr = xxl+dxx;
    xx0 = xxl+dxx/2;
    na = hist(dxy0(I000),xx0);
    nar = na./(xxr.^2-xxl.^2)/pi;
    pa = polyfit(xx0(xx0 >= rout_min & xx0 <= rout_max),nar(xx0 >= rout_min & xx0 <= rout_max)/sum(na),1);
    rout_mean = 2/3*(rout_max^3-rout_min^3)/(rout_max^2-rout_min^2);
    rin_mean = 2/3*r_th;
    r_ad1 = polyval(pa,rin_mean)/polyval(pa,rout_mean);
    r_ad_all(ii) = r_ad1;
    
    figure(3)
    subplot(sub_pos2(1),sub_pos2(2),ii)
        plot(xx0,nar/sum(na),'k','DisplayName','Data')
        hold on
        plot(xx0,polyval(pa,xx0),'r-.','DisplayName',['Fit (k = ',num2str(pa(1)),')'])
        xlim([0,xxl(end)-1])
        ylim([0,0.6])
        xlabel('r (um)')
        ylabel('%/um^2')
    ...
    Sp = pi*(rout_max^2-rout_min^2);
    nf = hist0(I_en0(I000 & dxy0 > rout_min & dxy0 <= rout_max),xl,xr,true);
    Pfake = nf/nnz(I111)*Sa/Sp/dw*r_ad1;
%     ffake = nf/sum(nf);
%     kf = sum(nf)/Sp/nnz(I111)*r_ad;
    
% %     nf0 = hist0(I_en0(I000 & dxy0 > rout_min & dxy0 <= rout_max),N_min,N_max,true);
% %     Pfake0 = nf0/nnz(I111)*Sa/Sp;
    
    nr = hist0(I_en0(I000 & dxy0 <= r_th),xl,xr,true);
