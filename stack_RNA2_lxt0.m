function stack_RNA2_lxt0(file_list,single_switch,CH_switch)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to stack_RNA_lx (see VARARGIN)
global imstack immask dlayer dcontrast dcontrmin dfsize dimthresh dmask dpeak mstatus resolution0 L_ratio g vlayer vcontrast vcontrmin vfsize vimthresh vmask vpeak limsize limthresh limcontrast bk_label sub_folder bk_name sg_name stack_th stack_th0 single_switch0 CH_switch0
...
% CH_switch0 = single_switch > 1;
single_switch0 = single_switch;
CH_switch0 = CH_switch;
if single_switch0 && (~CH_switch0)
    sub_folder = 'Histogram_A0/';
elseif single_switch0 && CH_switch0
    sub_folder = 'Histogram2_A0/';
elseif (~single_switch0) && (~CH_switch0)
    sub_folder = 'Histogram/';
else
    sub_folder = 'Histogram2/';
end
    hist_auto(file_list);
...
function load_file
global imstack imstack0 immask dlayer dcontrast dcontrmin dfsize dimthresh dmask dpeak mstatus resolution0 L_ratio g vlayer vcontrast vcontrmin vfsize vimthresh vmask vpeak limsize limthresh limcontrast max_spot n0 xout0 n xout imname imfolder bk_label bk_use mask_out stack_th single_switch0 CH_switch0 mask_stack
imstack = zeros(0);
imstack0 = zeros(0);
immask = false(0);
max_spot = zeros(0);
...
% % %     imstack(:,:,I_layer) = imfilter(temp,fspecial('gaussian',3,1),'symmetric','conv');
    imstack(:,:,I_layer) = imfilter(double(temp),-fspecial('log',10,2),'symmetric','conv');
...
% % % 
% % %     if xor(mean(mean(mean(imstack0(ymin1:ymax1,xmin1:xmax1,:)))) >= mean(mean(mean(imstack0(ymin2:ymax2,xmin2:xmax2,:)))),CH_switch0)
...
% else
    ...
    max_spot = ptrack0(imstack,mask_out);   %%% Track mRNA spot candidates
