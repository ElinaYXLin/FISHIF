function varargout = stack_protein_enrich(varargin)
% STACK_PROTEIN_ENRICH M-file for stack_protein_enrich.fig
%      STACK_PROTEIN_ENRICH, by itself, creates a new STACK_PROTEIN_ENRICH or raises the existing
%      singleton*.
%
%      H = STACK_PROTEIN_ENRICH returns the handle to a new STACK_PROTEIN_ENRICH or the handle to
%      the existing singleton*.
%
%      STACK_PROTEIN_ENRICH('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in STACK_PROTEIN_ENRICH.M with the given input arguments.
%
%      STACK_PROTEIN_ENRICH('Property','Value',...) creates a new STACK_PROTEIN_ENRICH or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before stack_protein_enrich_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to stack_protein_enrich_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help stack_protein_enrich

% Last Modified by GUIDE v2.5 03-Aug-2016 12:06:15

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @stack_protein_enrich_OpeningFcn, ...
                   'gui_OutputFcn',  @stack_protein_enrich_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before stack_protein_enrich is made visible.
function stack_protein_enrich_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to stack_protein_enrich (see VARARGIN)
global imstack immask dlayer dcontrast dcontrmin dfsize dimthresh dmask dpeak mstatus resolution0 L_ratio g vlayer vcontrast vcontrmin vfsize vimthresh vmask vpeak limsize limthresh limcontrast bk_label sub_folder RNA_folder bk_name sg_name stack_th stack_th0 channel2_add current_handle current_handle2
...
current_handle2 = get(handles.ana_type2,'SelectedObject');
...
set(handles.ana_type2,'SelectionChangeFcn',@ana_type2_SelectionChangeFcn);
...
% Choose default command line output for stack_protein_enrich
...
% end
% UIWAIT makes stack_protein_enrich wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = stack_protein_enrich_OutputFcn(hObject, eventdata, handles) 
...
rxyz = round([15,15,1]/L_ratio);
foci_area = repmat(getnhood(strel('disk',rxyz(1))),[1,1,2*(rxyz(3)+1)+1]);
...
try
    ...
    RNA_spot = xlsread([imfolder(1:find((imfolder(1:end-1) == '/') | (imfolder(1:end-1) == '\'),1,'last')),RNA_folder,xls_name2b]);
end
...
if isempty(strfind(imname,'_60X')) && isempty(strfind(file_list{I_file,2},'60X')) && isempty(strfind(file_list{I_file,2},airy_add)) && isempty(strfind(file_list{I_file,2},fast_add))
    [~,~,mismatch_matrix] = xlsread(mismatch_name);
    load(xymismatch_name)
    mismatch_name
elseif (~isempty(strfind(imname,'_60X')) || ~isempty(strfind(file_list{I_file,2},'60X'))) && isempty(strfind(file_list{I_file,2},airy_add)) && isempty(strfind(file_list{I_file,2},fast_add))
    [~,~,mismatch_matrix] = xlsread(mismatch_name2);
    load(xymismatch_name2)
    mismatch_name2
elseif (~isempty(strfind(imname,'_60X')) || ~isempty(strfind(file_list{I_file,2},'60X'))) && ~isempty(strfind(file_list{I_file,2},fast_add))
...
[x_start_im,y_start_im,x_center_im,y_center_im] = tile_info_extract([imfolder,imname,'/'],Nbin,imstack(:,:,1));
...
xyz00 = round(position_adjust(xyz00,imstack(:,:,1),protein_RNA_mismatch,protein_RNA_xymismatch,{x_start_im,y_start_im,x_center_im,y_center_im}));
xyz00(xyz00(:,1) > size(imstack,1),1) = size(imstack,1);
xyz00(xyz00(:,1) < 1,1) = 1;
xyz00(xyz00(:,2) > size(imstack,2),2) = size(imstack,2);
xyz00(xyz00(:,2) < 1,2) = 1;
xyz00(xyz00(:,3) > size(imstack,3),3) = size(imstack,3);
xyz00(xyz00(:,3) < 1,3) = 1;
...
mask00 = logical(convn_sparse(mask00,foci_area));
...
% %     [stack_th,t0,dt0] = th_find_low(spmask1(imstack,1) & ,lim,decrease_th,low_th);
    [mask_out,mask_out2D] = spmask1(imstack,0,stack_th,1,0);   %%% 3D local maximal mask generation (Primary filter)
...
%     [mask_out,mask_out2D] = spmask1(imstack,0,stack_th,1,0);   %%% 3D local maximal mask generation (Primary filter)
% % %     [mask_out,mask_out2D] = spmask1(imstack,0,stack_th,[],0);   %%% 3D local maximal mask generation (Primary filter)
...
%     mask_out2D(:,:,1:8) = false;
    ...
    peak_parameter = ptrack2(imstack0,imstack,mask_out,mask_out2D,0,[xrange,xrange],20);   %%% Track/fit mRNA spots
% % %     peak_parameter = ptrack(imstack0,imstack,mask_out,mask_out2D,0,[3,3]);   %%% Track/fit mRNA spots
...
global imstack immask dlayer dcontrast dcontrmin dfsize dimthresh dmask dpeak mstatus resolution0 L_ratio g vlayer vcontrast vcontrmin vfsize vimthresh vmask vpeak limsize limthresh limcontrast bk_label sub_folder bk_name sg_name stack_th stack_th0 channel2_add current_handle current_handle2
...
if strcmp(switch_reply,'Yes')
    stack_protein_enrich_OpeningFcn(hObject, eventdata, handles)
else
...
function ana_type2_SelectionChangeFcn(hObject, eventdata)
...
% %     children_handle = get(handles.ana_type2,'Children');
% %     current_handle2 = get(handles.ana_type2,'SelectedObject');
% %     next_handle = children_handle(children_handle ~= current_handle);
    set(handles.ana_type2,'SelectedObject',current_handle2)
end
%updates the handles structure
guidata(hObject, handles);
