function [peak_parameter,varargout] = ptrack3D(imstack0,imstack,mask_out,varargin)
tic
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
sigmaz0 = 2;   %%% Initial value of sigma_z
...
%         ydata1 = imstack(range11D)-all_offset(range11D);   %%%%% Collect the intensity values of data points
        ...
        xf30 = [vpeak(Nfit)-all_offset(Ipeak(Nfit))-min(ydata), 1./2./sigmax0.^2.*ones(size(Nfit)), X0(Nfit), 1./2./sigmay0.^2.*ones(size(Nfit)), Y0(Nfit), crossxy.*ones(size(Nfit)), min(ydata)*ones(size(Nfit)), 1./2./sigmaz0.^2.*ones(size(Nfit)), Z0(Nfit)];
...
% %         xf10 = [vpeak(Nfit)-all_offset(Ipeak(Nfit))-min(ydata), 1./2./sigmaz0.^2.*ones(size(Nfit)), Z0(Nfit), min(ydata)*ones(size(Nfit))];
        ...
        xf303D = [vpeak2D(Nfit2D)-all_offset(Ipeak2D(Nfit2D))-min(ydata), 1./2./sigmax0.^2.*ones(size(Nfit2D)), X02D(Nfit2D), 1./2./sigmay0.^2.*ones(size(Nfit2D)), Y02D(Nfit2D), crossxy.*ones(size(Nfit2D)), min(ydata)*ones(size(Nfit2D)), 1./2./sigmaz0.^2.*ones(size(Nfit2D)), Z02D(Nfit2D)];
...
% %         xf102D = [vpeak2D(Nfit2D)-all_offset(Ipeak2D(Nfit2D))-min(ydata), 1./2./sigmaz0.^2.*ones(size(Nfit2D)), Z02D(Nfit2D), min(ydata)*ones(size(Nfit2D))];
        ...
        [xf,resnorm,~,exitflag,~] = lsqcurvefit(gau3,[xf30;xf303D],xdata,ydata,lb,ub,options);
