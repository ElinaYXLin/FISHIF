function [kk,varargout] = NSX_binding_fit1(xdata,ydata,kk0,varargin)
...
% % %% kk0 (input): initial values of fitting parameters ((c0,h0,ki) for binding states 1 to N, kn0, and k01)
%% kk0 (input): initial values of fitting parameters ((c0,h0,ki,ki0) for binding states 1 to N, and k01)
...
c0 = kk0(1:4:end-1); c0l = zeros(size(c0)); c0u = c0*3;
h0 = kk0(2:4:end-1); h0l = h0; h0u = h0;
ki = kk0(3:4:end-1); kil = zeros(size(ki)); kiu = 20*ones(size(ki));
ki0 = kk0(4:4:end-1); ki0l = zeros(size(ki)); ki0u = 20*ones(size(ki));
k01 = kk0(end); k01l = 0; k01u = 20;

lb = zeros(size(kk0)); ub = zeros(size(kk0)); 
lb(1:4:end-1) = c0l; ub(1:4:end-1) = c0u; 
% lb(2:4:end-1) = h0l; ub(2:4:end-1) = h0u; 
lb(2:4:end-1) = zeros(size(h0l)); ub(2:4:end-1) = 2*h0u; 
lb(3:4:end-1) = kil; ub(3:4:end-1) = kiu; 
lb(4:4:end-1) = ki0l; ub(4:4:end-1) = ki0u; 
lb(end) = k01l; ub(end) = k01u;
...
lld = -sum(sum(log(NSX_binding1(kk,cx)).*Nx));
