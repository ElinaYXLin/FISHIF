function im_stitch_reference_pile_zshift(im1_path0,im2_path0,NucleusChannel1,NucleusChannel2)
...
FoldZshiftNum=['MatchTo_Zshift ',im1_path0(FoldRoot(end-1)+1:FoldRoot(end)-1)];
...
ZShiftTile_start=Z_start(:)-ShiftZ;
...
StackNumTile=length(imgDirN);
ZShiftTile_start(ZShiftTile_start<1)=1;
ZShiftTile_start(ZShiftTile_start>StackNumTile)=StackNumTile;
StackNumPile=StackNumTile-max(ZShiftTile_start);
...
for ti=1:size(TileName,1)
    ...
    TileI=0;
    for ii=ZShiftTile_start(ti):(ZShiftTile_start(ti)+StackNumPile)
        TileI=TileI+1;
