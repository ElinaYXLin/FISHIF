function Dual_all_add3(total_name)
...
for list_I = 1:N1
    ...
    for list_J = 1:M1        
        ...
        load([result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),mat_tail],'nucleus_RNA_profile','foci_RNA_profile','cytoplasmic_RNA_profile');   %%% load analysis result
%         load([folder_list{list_I,1},mask_folder,sub_list{list_J,3},mask_name]);   %%% load 3D mask
        
        [FISHI_temp,FISHN_temp,FISHI_temp2,FISHN_temp2] = RNA_profile3D_short(nucleus_RNA_profile,foci_RNA_profile,cytoplasmic_RNA_profile,['Sample ',num2str(N_plus)],N_cycle,flip_EL);   %%% RNA profile plot (5,6,8,11)
        
        out_name = cat(1,out_name,{image_folder});
        out_data = cat(1,out_data,[N_cycle,FISHI_temp2,FISHN_temp2]);
    end
end
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

out_data_all = cat(2,out_name,num2cell(out_data));
xlswrite([result_folder0,total_name,fish_add,output_tail],out_data_all);
