function [foci_data,fake_data,h,t_absolute,varargout] = dual_local_local(foci_bw00,max_image00,SS,foci_layer,mask_stack,signal_stack,RNA_stack,nucleus_protein_profile,image_folder,N_cycle,varargin)
...
%    mh{ir} = mh{ir}*sum(h{ir}(:))/sum(mh{ir}(:));
    ...
    bw0 = bwlabel(imerode(bwi,strel('disk',r_edge+r_size(ir))));
...
%        try
            ...
            f_ratio = sum(sum(bw01(x_all(I_ind)-x_h:x_all(I_ind)+x_h,y_all(I_ind)-y_h:y_all(I_ind)+y_h).*h{ir}))/sum(sum(bw01(x_all(I_ind)-mx_h:x_all(I_ind)+mx_h,y_all(I_ind)-my_h:y_all(I_ind)+my_h).*mh{ir}));
            f_mean(x_all(I_ind),y_all(I_ind),z_layer(x_all(I_ind),y_all(I_ind))) = sum(sum(signal_stack(x_all(I_ind)-mx_h:x_all(I_ind)+mx_h,y_all(I_ind)-my_h:y_all(I_ind)+my_h,z_layer(x_all(I_ind),y_all(I_ind))).*mh{ir}))*f_ratio;
%         catch err
%             I_ind
%             ir
%         end    
    ...
    f_mean = f_mean.*mask_stack;
    f_mean0 = max(f_mean,[],3);
    ...
    foci_data{ir}(:,3) = f_mean0(foci_bw00);
    fake_data{ir}(:,3) = f_mean0(fake_bw00);
