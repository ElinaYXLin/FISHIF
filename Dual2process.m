...
%         [nucleus_protein_profile,cytoplasmic_protein_profile] = protein_profile(seg_bw,cyto_bw,max_image,signal_channel,image_folder,N_cycle)
        ...
        figure(17)
            try
                ...
                imshow_lxt(double(max_image(:,:,RNA_channel))./max(max(double(max_image(:,:,RNA_channel)))));
            end
...
%%% Output: %%%============================================================
        ...
        if ~isempty(nucleus_RNA_profile)
            try
                ...
                xlwrite([result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),RNA_add,nu_add,output_tail],nucleus_RNA_profile);
            end
        end
        if ~isempty(cytoplasmic_RNA_profile)
            try
                ...
                xlwrite([result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),RNA_add,cyto_add,output_tail],cytoplasmic_RNA_profile);
            end
        end
        if ~isempty(foci_RNA_profile)
            try
                ...
                xlwrite([result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),RNA_add,foci_add,output_tail],foci_RNA_profile);
            end
        end
        if ~isempty(nucleus_protein_profile)
            try
                ...
                xlwrite([result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),protein_add,nu_add,output_tail],nucleus_protein_profile);
            end
        end
        if ~isempty(cytoplasmic_protein_profile)
            try
                ...
                xlwrite([result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),protein_add,cyto_add,output_tail],cytoplasmic_protein_profile);
            end
        end
