...
for list_I = 1:N1
    ...
    for list_J = 1%:M1%eval(run_list{list_I})
        ...
        [nucleus_protein_profile,cytoplasmic_protein_profile,quanti_p,nucleus_protein_ab] = protein_profile3D(max_image,protein_channel,mask_stack,signal_stack,image_folder,N_cycle,resolution);
...
%        variance_stack = nuclei_variance(mask_stack+1);
        nucleus_protein_profile_ab = [nucleus_protein_profile(:,1),nucleus_protein_ab];
...
%         signal_stack = pro_temp(mask_stack+1);
        ...
        [foci_data,fake_data,h,t_absolute,r_size] = dual_local_local3D3(foci_bw00,max_image_list,mask_stack,signal_stack,RNA_stack,nucleus_protein_profile_ab,image_folder,N_cycle,resolution,resolutionz);
        clear RNA_stack
        ...
        [nucleus_signal2_profile,foci_signal2_profile,cytoplasmic_signal2_profile] = RNA_profile3D(mask_stack,cyto_bw,foci_bw3D2,max_image002,SS2,signal2_stack,resolution,image_folder,N_cycle,[45,46,47,48,49,411]);   %%% Input the foci area matrix from modified_foci.m if gaussian fitting method is used
        ...
        [foci_data2,fake_data2,h2,t_absolute2,r_size2] = dual_local_local3D3(foci_bw002,max_image_list2,mask_stack,signal_stack,signal2_stack,nucleus_protein_profile_ab,image_folder,N_cycle,resolution,resolutionz,foci_mask,[473:477,480,481]);
...
%%% Output: %%%============================================================
        ...
        save([result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),mat_tail],'image_folder','image_type','WGA_channel','DAPI_channel','RNA_channel','protein_channel','resolution','seg_bw','cyto_bw','max_image','nucleus_RNA_profile','foci_RNA_profile','cytoplasmic_RNA_profile','nucleus_protein_profile','cytoplasmic_protein_profile','N_cycle','foci_data','fake_data','h','t_absolute','r_size','foci_data2','fake_data2','h2','t_absolute2','r_size2','resolutionz','quanti_p','-append');
