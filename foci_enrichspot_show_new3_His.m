...
I_plot = 1:10;%[3:5,8:11,22:25,27:34];%[3:5,8:11,14:19,22:25,27:34];
I_cycle = 1:12;
epsilon = 1e-4;
smin = 1e-1;
z_dim = 50;
...
result_folder = ['enrichspot_result/',embryo_name,'/analysis results/'];
...
for ii = 1:length(dxy0_cell)
    ...
    r_rescale = p0_protein(1)/p_protein(1);
    %%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    C_RNA0_cell{ii} = C_RNA0_cell{ii}(I_real,:); C_RNA0_cell{ii}(:,1:2) = C_RNA0_cell{ii}(:,1:2)*r_rescale;
...
% %     load([path0,em_name{ii}(1:end-1),mat_tail],'foci_data2','fake_data2','max_image','h','r_size','resolution','resolutionz','quanti_p');
    ...
    size0 = [size(max_image,1),size(max_image,2),z_dim];
...
% %     id0 = C_RNA0(:,IC) > 30e-9 & en_ring_ratio > smin;
    id0 = EL_RNA0 <=0.4 & EL_RNA0 >=0.15 & en_ring_ratio > smin;
    xdata = dxy0(id0);
...
% %     id0 = C_RNA0(:,IC) < 5e-9 & en_ring_ratio > smin;
    id0 = EL_RNA0 >= 0.7 & EL_RNA0 <= 0.85 & en_ring_ratio > smin;
    xdata = dxy0(id0);
    ydata = 1./en_ring_area(id0)./length(xdata);
...
set(gca,'Units','inches','Position',[2*dsize(1)+asize(1),dsize(2),asize])
    ...
    N0 = 0:dN:8;
%     id0 = C_RNA1(:,IC) > 10e-9;
    id0 = EL_RNA1 <= 0.4 & EL_RNA1 >= 0.15;
    [na,r0] = hist(N_en1(id0),N0);
%     id0 = C_RNA1(:,IC) < 5e-9;
    id0 = EL_RNA1 >= 0.7 & EL_RNA1 >= 0.85;
...
subplot(1,3,3)
set(gca,'Units','inches','Position',[3*dsize(1)+asize(1)*2,dsize(2),asize])
...
EL0 = [0,1];
N_range = 0;
bin_min = 0:0.025:0.95;   %%% bin_min for [Bcd]
bin_max = 0.05:0.025:1;   %%% bin_max for [Bcd]
C_range_mean0 = [0,9];

asize = [1,0.7];
asize2 = [0.4,0.4];
dsize = [1,1];
figsize = dsize+(dsize+asize).*[2,1];
...
set(gca,'Units','inches','Position',[dsize,asize])
    ...
    % % %     [xbin,ybin,xerr,yerr] = equal_bin(C_RNA1(I_all,IC)/1e-9,enrich_RNA(I_all,1),Nbin,rover,true);
        [xbin,ybin,xerr,yerr] = equal_dist(EL_RNA1(I_all),enrich_RNA(I_all,1),bin_min,bin_max);
    ...
    [xbin,ybin,xerr,yerr] = equal_dist(enrich_all(:,3),enrich_all(:,1),bin_min,bin_max);
...
axes
set(gca,'Units','inches','Position',[dsize+(dsize+asize).*[1,0],asize2])
...
% % set(gca,'Units','normalized')
    ...
    I_all = EL_RNA1 >= EL0(1) & EL_RNA1 <= EL0(2) & ~isnan(enrich_RNA(:,1)) & ismember(N_en1,N_range) & enrich_RNA(:,2) <= max(N_range);
    mean_En(1) = mean(enrich_RNA(I_all,1));
    std0_En(1) = std0(enrich_RNA(I_all,1));
    I_all = enrich_all(:,3) >= EL0(1) & enrich_all(:,3) <= EL0(2) & ~isnan(enrich_all(:,1));
...
Npeak = [1,2.4,4.2,6.5,8]';
Npeak0 = [0,1,2.4,4.2,6.5]';
Npeak1 = [2.4,4.2,6.5,8,9]';
% % Npeak = [1:8]';
% % Npeak0 = Npeak-1;
% % Npeak1 = Npeak+1;
...
cmin = [zeros(size(Npeak)),b0*Npeak-b0*(Npeak-Npeak0)*0.4,b0/5*ones(size(Npeak))]'; cmin = cmin(:)';
cmax = [1*ones(size(Npeak)),b0*Npeak+b0*(Npeak1-Npeak)*0.4,b0*1*ones(size(Npeak))]'; cmax = cmax(:)';
...
%%% Spot distance distribution vs. [TF]
    I000 = EL_RNA0 >= C_min(ii) & EL_RNA0 <= C_max(ii) & en_ring_ratio > smin;
    xdata = dxy0(I000);
    ...
    xlabel('r (\mum)','FontName',nfont,'FontSize',fsize,'FontWeight','normal')
    ylabel('P/\mum^2','FontName',nfont,'FontSize',fsize,'FontWeight','normal')
    title([num2str(C_min(ii)),' - ',num2str(C_max(ii)),' EL'],'Interpreter','none','FontName',nfont,'FontSize',fsize,'FontWeight','normal')
    set(gca,'FontName',nfont,'FontSize',fsize2,'FontWeight','normal')
    
%%% Spot intensity distribution vs. [TF]
    I000 = EL_RNA0 >= C_min(ii) & EL_RNA0 <= C_max(ii) & dxy0 > rout_min & dxy0 <= rout_max & Sp_en_ratio > smin & Sa_en_ratio > smin;
    I111 = EL_RNA1 >= C_min(ii) & EL_RNA1 <= C_max(ii) & Sp_foci_ratio > smin & Sa_foci_ratio > smin;
    I222 = EL_RNA0 >= C_min(ii) & EL_RNA0 <= C_max(ii) & dxy0 <= r_th & Sp_en_ratio > smin & Sa_en_ratio > smin;
    ...
    curve0 = fit(xx0(xx0 <= 5)',yy0(xx0 <= 5)', ['gauss',num2str(length(Npeak))], 'StartPoint',c0, 'Lower',cmin, 'Upper',cmax);
...
% %     curve0 = fit(xx0(xx0 <= 6)',yy0(xx0 <= 6)', ['gauss',num2str(length(Npeak))]);
    ...
    if mod(ii,Nbin2) == dN0
        figure(5)
        axes(ax0);
            plot(r0,Preal_all,'Color',ccode3(ceil(ii/Nbin2),:),'DisplayName',[num2str(C_mean(ii)),' EL']);
            hold on
...
axes
set(gca,'Units','inches','Position',[2*dsize(1)+asize(1),dsize(2),asize])
    plot(C_mean,Pon,'k.','DisplayName','Data')
hold on
    g0 = fittype( @(h1,c1,a1,h2,c2,a2,d, x) a1*x.^h1./(c1^h1+x.^h1)+a2*x.^h2./(c2^h2+x.^h2)+d );
    beta0 = [6,8,0.1,6,20,0.1,0];
    beta_min = zeros(size(beta0));
    beta_max = 2*beta0;% beta_max(end) = 1;
    curve = fit(C_mean',Pon', g0, 'StartPoint',beta0,'Lower',beta_min,'Upper',beta_max);
...
title('Spot Intensity vs. EL','Interpreter','none','FontName',nfont,'FontSize',fsize,'FontWeight','normal')
% legend('show')
set(gca,'FontName',nfont,'FontSize',fsize2,'FontWeight','normal','XTick',xtick2,'Units','normalized')
xlim(xlim5b)
ylim(ylim5b)
...
% % 
% % Icut_min = [0,0.6774,1.1894,1.5967,2.5290,2.7903];
% % Icut_max = [0.6774,1.1894,1.5967,2.5290,2.7903,3.2];
...
% % ybin = ybin/max(ybin);
% % curve1 = fit(xbin(xbin <= 4)',ybin(xbin <= 4)', ['gauss',num2str(length(Npeak))],'StartPoint',c0,'Lower',cmin,'Upper',cmax);
% % coeffvalues(curve1)
% % 
...
% %     for ii = 1:size(P_peak0,2)
% %         plot(C_mean/1e-9,P_peak0(:,ii)','Color',ccode(ii,:),'DisplayName',['State ',num2str(ii-1)])
% %         hold on
% %     end
% % xlim(xlim1)
% % ylim(ylim1)
