function varargout = nuclei_manual3D_liuliu(varargin)
% NUCLEI_MANUAL3D_LIULIU M-file for nuclei_manual3D_liuliu.fig
%      NUCLEI_MANUAL3D_LIULIU, by itself, creates a new NUCLEI_MANUAL3D_LIULIU or raises the existing
%      singleton*.
%
%      H = NUCLEI_MANUAL3D_LIULIU returns the handle to a new NUCLEI_MANUAL3D_LIULIU or the handle to
%      the existing singleton*.
%
%      NUCLEI_MANUAL3D_LIULIU('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in NUCLEI_MANUAL3D_LIULIU.M with the given input arguments.
%
%      NUCLEI_MANUAL3D_LIULIU('Property','Value',...) creates a new NUCLEI_MANUAL3D_LIULIU or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before nuclei_manual3D_liuliu_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to nuclei_manual3D_liuliu_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help nuclei_manual3D_liuliu

% Last Modified by GUIDE v2.5 22-Feb-2014 11:17:35

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @nuclei_manual3D_liuliu_OpeningFcn, ...
                   'gui_OutputFcn',  @nuclei_manual3D_liuliu_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before nuclei_manual3D_liuliu is made visible.
function nuclei_manual3D_liuliu_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to nuclei_manual3D_liuliu (see VARARGIN)

% Parameter setting:
set(gcf,'WindowButtonDownFcn',{}); set(gcf,'KeyPressFcn',{@KeyPressFcn0,hObject,eventdata,handles});
global sub_list sub_num in_folder input_name out_folder mask_name quick_mask_name mat_tail resolution0 image_folder DAPI_channel DAPI_th0 new_DAPI new_DAPI3D bw_DAPI bw_applied bw_applied3D sel_DAPI sel_applied L_ratio DAPI_on0 applied_on0 new_DAPI3D_save bw_applied3D_save v_save mask_stack layer_I layer_num  bw_cut area_thresh
in_folder = 'stacks/';
...
if ~exist([open_folder,out_folder,sub_list{J1,3},mask_name,mat_tail],'file')
    ...
    DAPI_seg3D2_liuliu({image_folder});
end
...
for image_I = 1:length(imlist)
    ...
    if image_I == 1
        new_DAPI3D = zeros(size(raw_im,1),size(raw_im,2),length(imlist));
    end
...
new_DAPI3D = max(new_DAPI3D,[],3);

layer_num = 1;%length(imlist);
[~,layer_I] = max(mean(mean(new_DAPI3D)));
...
% Choose default command line output for nuclei_manual3D_liuliu
...
bw_applied3D = logical(repmat(mask_stack(:,:,layer_I),[1,1,size(mask_stack,3)]));
...
% UIWAIT makes nuclei_manual3D_liuliu wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = nuclei_manual3D_liuliu_OutputFcn(hObject, eventdata, handles) 
...
if strcmp(quit_reply,'Yes')
    ...
    %%% Data loading
%     mask_stack = repmat(mask_stack(:,:,1),[1,1,size(mask_stack,3)]);
    bw_applied3D_save = repmat(bw_applied3D_save(:,:,1),[1,1,size(bw_applied3D_save,3)]);
...
if get(handles.current_seg,'Value')
    ...
    elseif get(handles.sel_single,'Value')
        ...
        bw_DAPI_single = find(([bw_DAPI_prop.MaxIntensity] == [bw_DAPI_prop2.MinIntensity]) & ([bw_DAPI_prop.MaxIntensity] > [bw_DAPI_prop.MinIntensity]));
...
%     sel_DAPI = bw_DAPI0 | sel_DAPI;
    ...
    if get(handles.auto_cut,'Value')
        ...
        r0 = max(round(mean([bw_applied_prop.EquivDiameter])/2*0.3),10);
    ...
    % %         bw_applied_single = find(([bw_applied_prop3.MeanIntensity] == 1);
            ...
            bw_DAPI_single = find([bw_DAPI_prop3.MeanIntensity] < 0.90);
...
if strcmp(quit_reply,'Yes')
    ...
    bw_applied3D(:,:,layer_I) = bw_applied;
    bw_applied3D = repmat(bw_applied3D(:,:,1),[1,1,size(bw_applied3D,3)]);
    bw_applied3D_save(v_save(3):v_save(4),v_save(1):v_save(2),:) = bw_applied3D;
...
% --- Executes on button press in syn_mask.
function syn_mask_Callback(hObject, eventdata, handles)
% hObject    handle to syn_mask (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global sub_list sub_num in_folder input_name out_folder mask_name quick_mask_name mat_tail resolution0 image_folder DAPI_channel DAPI_th0 new_DAPI new_DAPI3D bw_DAPI bw_applied bw_applied3D sel_DAPI sel_applied L_ratio DAPI_on0 applied_on0 new_DAPI3D_save bw_applied3D_save v_save mask_stack layer_I layer_num  bw_cut
set(gcf,'WindowButtonDownFcn',{}); set(gcf,'KeyPressFcn',{@KeyPressFcn0,hObject,eventdata,handles});
quit_reply = questdlg(['Copy mask from layer ',num2str(layer_I),' to all layers?']);
if strcmp(quit_reply,'Yes')
    set(handles.syn_mask,'String','Wait');
    
    bw_applied3D(:,:,layer_I) = bw_applied;
    bw_applied3D_save(v_save(3):v_save(4),v_save(1):v_save(2),:) = bw_applied3D;
    bw_applied3D = repmat(bw_applied3D(:,:,layer_I),[1,1,size(bw_applied3D,3)]);
    bw_applied3D_save = repmat(bw_applied3D_save(:,:,layer_I),[1,1,size(bw_applied3D_save,3)]);
    
    % Update handles structure
    guidata(hObject, handles);
    set(handles.syn_mask,'String','Sync Mask');
end
%%% Update handles structure
guidata(hObject, handles);
