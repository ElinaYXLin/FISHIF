...
% % %         mask_expand = mask_stack;
        x00 = min(max(round(foci_list(:,6)),1),size(mask_stack,1)); Nx00 = round(foci_list(:,6)) >= 1 & round(foci_list(:,6)) <= size(mask_stack,1);
        y00 = min(max(round(foci_list(:,7)),1),size(mask_stack,2)); Ny00 = round(foci_list(:,7)) >= 1 & round(foci_list(:,7)) <= size(mask_stack,2);
        z00 = min(max(round(foci_list(:,8)),1),size(mask_stack,3)); Nz00 = round(foci_list(:,8)) >= 1 & round(foci_list(:,8)) <= size(mask_stack,3);
        Nuclei_foci = mask_expand(sub2ind(size(mask_stack),x00,y00,z00));
