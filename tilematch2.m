function [outimage,IImatch,mismatch] = tilematch2(input1,input2,Mdim,match_channel,prematch,compare_ratio)
...
match_ratio = ones(Nmatch,Nsize1(3)); %%% mean intensity ratio
%%%========================================================================
...
for Imatch = match_start:match_end
    ...
    temp10 = input1(input1_start(1):input1_end(1),input1_start(2):input1_end(2),:);
    temp20 = input2(input2_start(1):input2_end(1),input2_start(2):input2_end(2),:);
    match_mask = im2bw(uint16(temp1),mthreshold*graythresh(uint16(temp1)))|im2bw(uint16(temp2),mthreshold*graythresh(uint16(temp2)));
    for I_chan = 1:Nsize1(3)
        match_ratio(Imatch,I_chan) = sum(sum(double(temp10(:,:,I_chan))))/sum(sum(double(temp20(:,:,I_chan))));
    end
    raw_mismatch = (double(temp1)-match_ratio(Imatch,I_chan)*double(temp2))./(double(temp1)+match_ratio(Imatch,I_chan)*double(temp2)+((double(temp1)+match_ratio(Imatch,I_chan)*double(temp2))<=0));
...
for I_chan = 1:Nsize1(3)
    outimage(:,:,I_chan) = cat(Mdim,uint16(input1(input0_start(1):input0_end(1),input0_start(2):input0_end(2),I_chan)),uint16(match_ratio(IImatch,I_chan)*input2(input3_start(1):input3_end(1),input3_start(2):input3_end(2),I_chan)));
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%