%function foci_bw = RNA_seg2(seg_bw,max_image,signal_channel,WGA_channel,resolution,image_folder,N_cycle)
...
%%% Selection of the threshold as the first local maxima of the second order gradient:
    g_max = imregionalmax(diff(fit_g));
    g_max(1) = 0;
    II_max = intersect(find(g_max),find(Ng < Ng_th)-fit_radius);
    if ~isempty(II_max)
        Vpmax(Ip) = fit_g(min(II_max));
...
title(['Threshold - # profile and the selected threshold value (',image_folder,') ','cycle = ',num2str(N_cycle),', threshold = ',num2str(g_threshold0),', pixel size = ',num2str(psize(Ip0))])
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
new_image(:,:,1) = max_image(:,:,signal_channel);
new_image(:,:,2) = 0;
new_image(:,:,3) = 0;
