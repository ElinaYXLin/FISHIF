function DAPI_seg3D2(lf_name,varargin)
%clear all
close all
tic
...
sigma_th = 150;  %%% neighboring mask threshold correlation decay length for supplemental mask recognition
% rsigma_th = 100;  %%% neighboring mask threshold correlation decay length for supplemental mask recognition
% % % back_th = 1.5;
pcore = 0.5;
...
%%% Methods to locate nuclei and determine the local threshold:     0. Layer-wise detection using criterion of circles; ...
                                                                 ...0.1. Layer-wise detection using local thresholds derived from criterion of circles; ...
                                                                 ...
                                                                 ...2.1. Location using maximum projected image (with mean intensity threshold) and detection using local thresholds derived from criterion of circles; ...
...
if length(varargin)>= 12 && ~isempty(varargin{12}) 
    th_ratio0 = varargin{12};
else
    th_ratio0 = 1.0;   %%% threshold resetting ratio for mask refinement on z direction
end
...
% %         th_ratio = 0.4;
        ...
        if t_reuse == 2 || t_reuse == 2.1 || t_reuse == 3 || t_reuse == 4 || t_reuse == 5
            S = load([folder_list{list_I,1},result_folder,sub_list{list_J,3}(1:end-1),mat_tail],'max_image');
            raw_im = S.max_image; clear S
            if scale0 < 1
                raw_im = imresize(raw_im(1:end-n1,1:end-n2,:),scale0);
        ...
        if t_reuse == 2 || t_reuse == 2.1 || t_reuse == 4 || t_reuse == 5
...
% %             bw_diff = imopen(bw_diff,strel('disk',10));
            bw_diff = imopen(bw_diff,strel('disk',5));
...
% %             raw_im_all = zeros(0);
% %             for image_I0 = image_I-3:image_I+3
% %                 raw_im0 = imread([folder_list{list_I,1},in_folder,sub_list{list_J,3},imlist(image_I).name]);
% %                 raw_im_all = cat(4,raw_im_all,raw_im0);
% %             end
% %             raw_im = uint16(mean(raw_im_all,4));
...
%% Supplement segmentation based on center recognition: %%%%%%%%%%%%%%%%%%%
            ...
            if ~isempty(bw_props) && t_reuse && t_reuse ~= 0.1 && t_reuse ~= 2.1 
...
% % %                         bw_th = bwareaclose(fil_im >= th_estimate*65535,merge_th);
% % %                         bw_th = bw_th | bwareaclose(~bw_th,200);
...
% %                     bw_th = imclose(fil_im >= th_ratio*th_2D(1)*65535,strel('disk',10));
                    bw_th = imopen(fil_im >= th_ratio*th_2D(1)*65535,strel('disk',3));
                    bw_th_label = bwlabel(bw_th);
...
% %                 bw_out = imfill(bwareaopen(bwareaclose(fil_im >= 1.0*bw_th*65535,merge_th),low_th),'holes');
                ...
                bw_out = bwareaopen(imopen(bw_out,strel('disk',2)),low_th);
                if r_close(2) > 0
                    bw_out = imclose(bw_out,strel('disk',r_close(2)));
                    if t_reuse ~= 5
                        bw_out = imfill(bw_out,'holes');
% % %                         bw_out = bw_out | bwareaclose(~bw_out,100);
                    else
% %                         bw_out = imfill(bw_out,'holes');
% %                         bw_out = imclose(bw_out,strel('disk',5));
% %                         bw_out = imfill(bw_out,'holes');
                        bw_out = bw_out | bwareaclose(~bw_out,100);
                    end
                end
