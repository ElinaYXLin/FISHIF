function varargout = stack_RNA_lx(varargin)
% STACK_RNA_LX M-file for stack_RNA_lx.fig
%      STACK_RNA_LX, by itself, creates a new STACK_RNA_LX or raises the existing
%      singleton*.
%
%      H = STACK_RNA_LX returns the handle to a new STACK_RNA_LX or the handle to
%      the existing singleton*.
%
%      STACK_RNA_LX('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in STACK_RNA_LX.M with the given input arguments.
%
%      STACK_RNA_LX('Property','Value',...) creates a new STACK_RNA_LX or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before stack_RNA_lx_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to stack_RNA_lx_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help stack_RNA_lx

% Last Modified by GUIDE v2.5 09-Sep-2012 18:16:44

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @stack_RNA_lx_OpeningFcn, ...
                   'gui_OutputFcn',  @stack_RNA_lx_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before stack_RNA_lx is made visible.
function stack_RNA_lx_OpeningFcn(hObject, eventdata, handles, varargin)
...
% Choose default command line output for stack_RNA_lx
...
% end
% UIWAIT makes stack_RNA_lx wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = stack_RNA_lx_OutputFcn(hObject, eventdata, handles) 
...
if strcmp(switch_reply,'Yes')
    stack_RNA_lx_OpeningFcn(hObject, eventdata, handles)
else
