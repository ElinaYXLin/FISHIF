...
I_plot = [1,3,5,6,11];%[1,2,5,8,9,10];%[3:5,8:11,22:25,27:34];%[3:5,8:11,14:19,22:25,27:34];
I_cycle = 10:12;
epsilon = 1e-4;
smin = 1e-1;
rsize0 = 8;
...
subplot(1,4,2)
set(gca,'Units','inches','Position',[2*dsize(1)+1*asize(1),dsize(2),asize])
...
subplot(1,4,4)
set(gca,'Units','inches','Position',[4*dsize(1)+asize(1)*3,dsize(2),asize])
...
sub_pos2 = [5,10];
Nbin = prod(sub_pos2); Nbin2 = 11; dN0 = 2;
rover = 0.4;
...
cmin = [zeros(size(Npeak)),b0*Npeak-b0*(Npeak-Npeak0)*0.5,b0/3*ones(size(Npeak))]'; cmin = cmin(:)';
cmax = [1*ones(size(Npeak)),b0*Npeak+b0*(Npeak1-Npeak)*0.5,b0*3*ones(size(Npeak))]'; cmax = cmax(:)';
...
%             plot(r0,Preal_all,'k.');
        ...
        ylabel('Frequency','FontName',nfont,'FontSize',fsize,'FontWeight','normal')
        title(['EL: ',num2str(EL_mean(ii)),', C = ',num2str(C_mean(ii)/1e-9),' nM'])
        set(gca,'FontName',nfont,'FontSize',fsize2,'FontWeight','normal')
        box off
        view(90,-90)
    end
...
xlim2 = [0,1];
ylim2 = [0,0.3;0,0.1;0,0.2];
EL_range0 = [0,0.8;0,1;0,1];
beta00 = {[0.02,0.3,0.1,0.02,0.7,0.1,0],[0.05,0.5,0.1,0],[-0.02,0.1,0.03,-0.02,0.3,0.03,0]};
...
for ii = 1:length(plotN)
    axes('Units','inches','Position',[dsize2+(dsize2+asize4).*[0,ii-1],asize4]);
        ...
        plot(xx1,yy1,'k.')
        hold on
        if any(ismember(plotN{ii},[2,4,5]))
            g0 = fittype( @(h1,c1,a1,h2,c2,a2,d, x) a1*exp(+(x-c1)./h1)./(1+exp(+(x-c1)./h1))+a2*exp(+(x-c2)./h2)./(1+exp(+(x-c2)./h2))+d );
            beta0 = beta00{ii};
