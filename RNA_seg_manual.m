%function foci_bw = RNA_seg_manual(seg_bw,max_image,signal_channel,WGA_channel,resolution,image_folder,N_cycle)
...
%%%%% Plot of the profile with the selected threshold: %%%%%---------------
    ...
    title(['Selection of threshold value (',image_folder,') ','cycle = ',num2str(N_cycle),', threshold = ',num2str(g_threshold0),', pixel size = ',num2str(psize(Ip0)),', click "n" if no more selection is needed'])
%%%%%----------------------------------------------------------------------
        [Ix,~,button] = ginput(1);
        if any(button == 'nN')
            g_manual = false;
