...
I_plot = 1:10;%[1:5,7,9];%[1,2,5,8,9,10];%[3:5,8:11,22:25,27:34];%[3:5,8:11,14:19,22:25,27:34];
I_cycle = 9:11;
epsilon = 1e-4;
smin = 1e-1;
smin2 = 0.99;
...
% %     r_rescale = r_rescale*r_rescale0(ii);
    ...
    I_real_RNA = z000 >= zdiff_nucleus_RNA(1) & z000 <= zdiff_nucleus_RNA(2);
...
angle_bin0 = -pi:0.04*pi:pi; angle_bin = (angle_bin0(1:end-1)+angle_bin0(2:end))/2;
xlim1 = [0,0.8];
% ylim1 = [0,0.6];
ylim1 = [0,15];
ylim1b = [0,150];
...
% %     id0 = C_RNA0(:,IC) > 30e-9 & en_ring_ratio > smin;
    id0 = EL_RNA0 >= 0.20 & EL_RNA0 <= 0.4 & I_RNA0 <= N_thresh & en_ring_ratio > smin;
    id1 = EL_RNA1 >= 0.20 & EL_RNA1 <= 0.4 & I_RNA1 <= N_thresh;
    xdata = dxy0(id0);
...
% %     id0 = C_RNA0(:,IC) < 5e-9 & en_ring_ratio > smin;
    id0 = EL_RNA0 >= 0.6 & EL_RNA0 <= 0.8 & I_RNA0 <= N_thresh & en_ring_ratio > smin;
    id1 = EL_RNA1 >= 0.6 & EL_RNA1 <= 0.8 & I_RNA1 <= N_thresh;
    xdata = dxy0(id0);
    ydata = 1./en_ring_area(id0)./nnz(id1)/resolutionz/length(dz0);%./length(xdata);
...
axes('Units','inches','Position',[3*dsize(1)+2*asize(1),dsize(2),asize])
    ...
    N0 = 0:dN:8;
%     id0 = C_RNA1(:,IC) > 10e-9;
    id0 = EL_RNA1 >= 0.2 & EL_RNA1 <= 0.4 & I_RNA1 <= N_thresh;
    [na,r0] = hist(N_en1(id0),N0);
%     id0 = C_RNA1(:,IC) < 5e-9;
% %     [np,r0] = hist(N_en2(id0),N0);
% % %     id0 = EL_RNA1 >= 0.10 & EL_RNA1 <= 0.35;
% % %     [np,r0] = hist(N_en2(id0),N0);
    id0 = EL_RNA1 >= 0.6 & EL_RNA1 <= 0.8 & I_RNA1 <= N_thresh;
...
for ii = 1:size(EL0,1)
    ...
    P_th0 = hist(N_en2(I111),bin0); P_th1 = P_th0/sum(P_th0);
    P_out0 = hist(N_en1(I111),bin0); P_out1 = P_out0/sum(P_out0);
...
sub_pos2 = [5,10];
Nbin = prod(sub_pos2); Nbin2 = 5; dN0 = 3;
rover = 0.4;
...
Npeak0 = [4*(0:19)]'-0.33;
Npeak1 = [4*(2:21)]'-0.33;
% Npeak = [1,2.5,5,7.5,10,12.5,15,17.5]';
% Npeak0 = [0,1,2.5,5,7.5,10,12.5,15]';v
% Npeak1 = [2.5,5,7.5,10,12.5,15,17.5,20]';
% % Npeak = [1:8]';
% % Npeak0 = Npeak-1;
% % Npeak1 = Npeak+1;

Il0 = Npeak-0.50*(Npeak(2)-Npeak(1)); Il0(1) = 0;
Ir0 = Npeak+0.50*(Npeak(2)-Npeak(1));
...
b0 = 1; b1 = 4;
c0 =[ones(size(Npeak))*0.1,b0*Npeak,b1/4*ones(size(Npeak))]'; c0 = c0(:)';
cmin = [zeros(size(Npeak)),b0*Npeak-b0*(Npeak-Npeak0)*0.5,b1/8*ones(size(Npeak))]'; cmin(1,2) = 0; cmin = cmin(:)';
cmax = [1*ones(size(Npeak)),b0*Npeak+b0*(Npeak1-Npeak)*0.5,b1/2*ones(size(Npeak))]'; cmax = cmax(:)';
...
% % %     ydata = 1./Sp_en(I000);
% % %     [~,nf,~] = histw(xdata,ydata,Il,Ir);
% % %     Pfake_all = nf/nnz(I111)*mean(Sa_foci(I111))/dw;
% % %     %%%%%
...
%%% Preal vs. [TF]    
    I111 = EL_RNA1 >= EL_min(ii) & EL_RNA1 <= EL_max(ii) & Sp_foci_ratio > smin & Sa_foci_ratio > smin;
    f_bk = mean((N_outmax(I111)-N_outmin(I111))./Sp_foci(I111));
...
% %     g0 = fittype( @(h1,c1,a1,h2,c2,a2,d, x) a1*exp(+(x-c1)./h1)./(1+exp(+(x-c1)./h1))+a2*exp(+(x-c2)./h2)./(1+exp(+(x-c2)./h2))+d );
    g0 = fittype( @(h1,c1,a1,h2,c2,a2,d, x) a1*exp(-(x-c1)./h1)./(1+exp(-(x-c1)./h1))+a2*exp(-(x-c2)./h2)./(1+exp(-(x-c2)./h2))+d );
%     g1 = fittype (@(a,b,c,d,x) c*exp(-x/a)./(exp(-x/a)+exp(-b/a))+d);
%     beta0 = [6,0.4,0.1,12,0.7,0.1,0];
    beta0 = [0.03,0.2,0.1,0.1,0.8,0.1,0];
    beta_min = [0,0,0,0,0.6,0,0];
    beta_max = [1,0.4,0.2,1,1,0.2,0];
% %     beta_max = 2*beta0;% beta_max(end) = 1;
    I_true0 = EL_mean >= 0 & EL_mean <= 1;
    curve = fit(EL_mean(I_true0)',Pon(I_true0)', g0, 'StartPoint',beta0,'Lower',beta_min,'Upper',beta_max);
    beta1 = coeffvalues(curve);
    plot(EL_line,feval(curve,EL_line),'r-','DisplayName','Fit');
xlabel('EL','FontName',nfont,'FontSize',fsize,'FontWeight','normal')
ylabel('Pon','FontName',nfont,'FontSize',fsize,'FontWeight','normal')
title(['Spot frequency vs. EL: beta = ',num2str(beta1)],'Interpreter','none','FontName',nfont,'FontSize',fsize,'FontWeight','normal')
legend('show')
...
b0 = 1; b1 = 4;
c0 =[0.1*ones(size(Npeak)),b0*Npeak,b1/4*ones(size(Npeak))]'; c0 = c0(:)';
cmin = [zeros(size(Npeak)),b0*Npeak-b0*(Npeak-Npeak0)*0.5,b1/10*ones(size(Npeak))]'; cmin = cmin(:)';
cmax = [2*ones(size(Npeak)),b0*Npeak+b0*(Npeak1-Npeak)*0.5,b1/2*ones(size(Npeak))]'; cmax = cmax(:)';
...
% % % % Icut_min = [0,14,22,29,36,41,56,63];
% % % % Icut_max = [14,22,29,36,41,56,63,80];
...
ybin = ybin/max(ybin);
c0(1:3:end) = local_max(xx0,yy0,(Npeak+Npeak0)/2,(Npeak+Npeak1)/2);
...
%         plot(C_mean/1e-9,I_peak(:,ii)','Color',ccode2(ii,:),'DisplayName',['State ',num2str(ii-1)])
...
xdata2 = EL_mean(I_true0)';
ydata2 = P_peak(I_true0,1:end);
ydata2(:,1) = 1-ydata2(:,1);
ydata = ydata2.*repmat(N_mean(I_true0)',1,size(ydata2,2));

% % clim = [0,20;8,30;30,80;40,80;60,100];
% % % clim = [0,200;0,200;0,200;0,200];
% % xlim1c = [0.7,1.3;0.8,1.8;1.5,1.9;1.6,2;1.7,2.1];
% % ylim1c = [-2,0;-1,1;0,1;-0.5,0.5;-1,0];
% % cplot = 0:0.01:3;
% % 
% % N1 = 1:5;
% % N2 = 2:6;
...
% % set(gcf,'Units','inches','Position',[1,1,figsize2])
% % for ii = 1:length(N1)
% %     I_true2 = xdata >= clim(ii,1) & xdata <= clim(ii,2);
% %     x00 = log10(xdata);
...
% % I_true2 = xdata <= 100; I_true2([9,15:17]) = false;
...
xlim2 = [0,1];
ylim2 = [0,0.03;0,0.03;0,0.03;0,0.03;0,0.03;0,0.03];
EL_range0 = [0.4,0.7;0,0.6;0.3,0.8;0,1;0,1;0,1];

g0 = fittype( @(h1,c1,a1,d, x) a1*exp(-(x-c1)./h1)./(1+exp(-(x-c1)./h1))+d );
beta00 = {[0.05,0.6,0.1,0],[0.05,0.2,0.05,0],[-0.05,0.5,0.02,0],[0.05,0.2,0.05,0]};
...
% % % % %                 beta_max = [0,1,1,0];
% % % % %             end
% % %         I_true0 = EL_mean >= EL_range0(ii,1) & EL_mean <= EL_range0(ii,2);
% % %         curve = fit(xx1(I_true0),yy1(I_true0), g0, 'StartPoint',beta0,'Lower',beta_min,'Upper',beta_max);
% % %         plot(EL_line,feval(curve,EL_line),'r-','DisplayName','Fit');
    xlim(xlim2)
    ylim(ylim2(ii,:))
% %     if any(ismember(plotN{ii},1))
% % %         title(['N = ',num2str(plotN{ii}),', kn0 = ',num2str(kn0),'k01 = ',num2str(k01)])
% %         title(['N = ',num2str(plotN{ii}),', k01 = ',num2str(k01)])
% %     else
% % %         title(['N = ',num2str(plotN{ii}),', C0 = ',num2str(c0(plotN{ii}-1)),'h0 = ',num2str(h0(plotN{ii}-1))])
% %         title(['N = ',num2str(plotN{ii}),', C0 = ',num2str(c0(plotN{ii}-1)),', h = ',num2str(h0(plotN{ii}-1)),', ki = ',num2str(ki(plotN{ii}-1)),', ki0 = ',num2str(ki0(plotN{ii}-1))])
        title(['N = ',num2str(plotN{ii})])
% %     end

    xlabel('AP position (EL)','FontName',nfont,'FontSize',fsize,'FontWeight','normal')
    ylabel('Probability','FontName',nfont,'FontSize',fsize,'FontWeight','normal')
    set(gca,'FontName',nfont,'FontSize',fsize2,'FontWeight','normal')
    box off
    set(gca,'Units','normalized')
...
asize4 = [1.5,0.6];
dsize2 = [0.5,0.5];
figsize3 = dsize2+(dsize2+asize4).*[1,2];

figure(100)
set(gcf,'Units','inches','Position',[1,1,figsize3])

his_mean = ydata2*Npeak00;
his_var = ydata2*(Npeak00.^2)-his_mean.^2;
his_std = sqrt(his_var);
his_std0 = his_std;%./sqrt(N_mean');
his_fano = his_var./his_mean;

ldata2 = his_mean-his_std0;
udata2 = his_mean+his_std0;

axes('Units','inches','Position',[dsize2+(dsize2+asize4).*[0,0],asize4]);
yyaxis left
    plot(xdata2,his_mean,'g')
    hold on
    patch([xdata2;xdata2(end:-1:1)],[ldata2;udata2(end:-1:1)],'k','FaceColor',[0.8,1,0.8],'EdgeColor','none');
xlim(xlim3)
ylim(ylim3a)
xlabel('AP position (EL)','FontName',nfont,'FontSize',fsize,'FontWeight','normal')
ylabel('I_m_e_a_n','FontName',nfont,'FontSize',fsize,'FontWeight','normal')
title('All signal','FontName',nfont,'FontSize',fsize,'FontWeight','normal')
    
yyaxis right
    plot(xdata2,his_fano,'k')
xlim(xlim3)
ylim(ylim3b)
ylabel('Fano factor','FontName',nfont,'FontSize',fsize,'FontWeight','normal')
box off

%%%
his_mean2 = ydata2(:,2:end)*Npeak00(2:end)./sum(ydata2(:,2:end),2);
his_var2 = ydata2(:,2:end)*(Npeak00(2:end).^2)./sum(ydata2(:,2:end),2)-his_mean2.^2;
his_std2 = sqrt(his_var2);
his_std20 = his_std2;%./sqrt(N_mean');
his_fano2 = his_var2./his_mean2;

ldata2 = his_mean2-his_std20;
udata2 = his_mean2+his_std20;

axes('Units','inches','Position',[dsize2+(dsize2+asize4).*[0,1],asize4]);
yyaxis left
    plot(xdata2,his_mean2,'g')
    hold on
    patch([xdata2;xdata2(end:-1:1)],[ldata2;udata2(end:-1:1)],'k','FaceColor',[0.8,1,0.8],'EdgeColor','none');
xlim(xlim3)
ylim(ylim3c)
xlabel('AP position (EL)','FontName',nfont,'FontSize',fsize,'FontWeight','normal')
ylabel('I_m_e_a_n','FontName',nfont,'FontSize',fsize,'FontWeight','normal')
title('ON signal','FontName',nfont,'FontSize',fsize,'FontWeight','normal')
    
yyaxis right
    plot(xdata2,his_fano2,'k')
xlim(xlim3)
ylim(ylim3d)
ylabel('Fano factor','FontName',nfont,'FontSize',fsize,'FontWeight','normal')
box off
...
%%
saveas(100,[result_folder,out_name,binding_add,mean_add,fit_add2,fig_tail])
saveas(100,[result_folder,out_name,binding_add,mean_add,fit_add2,fig_tail2],'epsc')
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
