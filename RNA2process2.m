...
hist2_folder_single = 'Histogram_A_RNA2/';
fit_tail = '_raw';
output_tail = '.xls';
output_tail2 = '.xlsx';
figure_tail = '.fig';
...
xymismatch_name = 'xymismatch_60X';
mismatch_name = 'mismatch_60X';
% % xymismatch_name = 'xymismatch_60X_FA';
% % mismatch_name = 'mismatch_60X_FA';
% % % % xymismatch_name = 'xymismatch_60X_old';
% % % % mismatch_name = 'mismatch_60X_old';
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% %         RNA2_channel = sub_num(list_J,10);
        RNA_channel = sub_num(list_J,10);
        RNA2_channel = sub_num(list_J,12);
        resolution = sub_num(list_J,9);

        if exist([folder_list{list_I,1},hist_folder,sub_list{list_J,3}(1:end-1),fit_tail,output_tail])
            RNA_fit = xlsread([folder_list{list_I,1},hist_folder,sub_list{list_J,3}(1:end-1),fit_tail,output_tail]);
        else
            RNA_fit = xlsread([folder_list{list_I,1},hist_folder,sub_list{list_J,3}(1:end-1),fit_tail,output_tail2]);
        end
        if exist([folder_list{list_I,1},hist2_folder,sub_list{list_J,3}(1:end-1),fit_tail,output_tail])
            RNA2_fit = xlsread([folder_list{list_I,1},hist2_folder,sub_list{list_J,3}(1:end-1),fit_tail,output_tail]);
        else
            RNA2_fit = xlsread([folder_list{list_I,1},hist2_folder,sub_list{list_J,3}(1:end-1),fit_tail,output_tail2]);
        end
        if exist([folder_list{list_I,1},hist_folder_single,sub_list{list_J,3}(1:end-1),fit_tail,output_tail])
            RNA_fit_single = xlsread([folder_list{list_I,1},hist_folder_single,sub_list{list_J,3}(1:end-1),fit_tail,output_tail]);
        else
            RNA_fit_single = xlsread([folder_list{list_I,1},hist_folder_single,sub_list{list_J,3}(1:end-1),fit_tail,output_tail2]);
        end
        if exist([folder_list{list_I,1},hist2_folder_single,sub_list{list_J,3}(1:end-1),fit_tail,output_tail])
            RNA2_fit_single = xlsread([folder_list{list_I,1},hist2_folder_single,sub_list{list_J,3}(1:end-1),fit_tail,output_tail]);
        else
            RNA2_fit_single = xlsread([folder_list{list_I,1},hist2_folder_single,sub_list{list_J,3}(1:end-1),fit_tail,output_tail2]);
        end
...
%         [length(RNA_fit_single),length(RNA2_fit_single),length(single_mismatch)]
        ...
        all_size = cat(1,all_size,{size(max_image)});
%%% Output: %%%============================================================
        ...
        save([result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),mat_tail],'foci_mismatch','single_mismatch','RNA_channel','RNA2_channel','-append');
...
% %     xy2a = zeros(0);
    ...
    for ii = 1:length(all_foci)
        ...
        z2 = cat(1,z2,temp(:,13));
% %         tempa = all_single{ii};
% %         xy1a = cat(1,xy1a,tempa(:,8:9)-repmat((all_size{ii}(1:2)+[1,1])/2,size(tempa,1),1));
% %         xy2a = cat(1,xy2a,tempa(:,11:12)-repmat((all_size{ii}(1:2)+[1,1])/2,size(tempa,1),1));
    end
...
% %     fit2Dxa = polyfitn(xy1a,xy2a(:,1),1);
% %     fit2Dya = polyfitn(xy1a,xy2a(:,2),1);
% %     M0a = [fit2Dxa.Coefficients;fit2Dya.Coefficients];
% %     
% %     [channel_name{RNA2_channel},'_',channel_name{RNA_channel}]
% %     M0a-M0
...
% %     fit2Dxa = polyfitn(xy2a,xy1a(:,1),1);
% %     fit2Dya = polyfitn(xy2a,xy1a(:,2),1);
% %     M0a = [fit2Dxa.Coefficients;fit2Dya.Coefficients];
% %     
% %     [channel_name{RNA_channel},'_',channel_name{RNA2_channel}]
% %     M0a-M0
