...
sub_pos2 = [5,10];
Nbin = prod(sub_pos2); Nbin2 = 10; dN0 = 5;
rover = 0.4;
...
cmin = [zeros(size(Npeak)),b0*Npeak-b0*(Npeak-Npeak0)*0.6,b0/3*ones(size(Npeak))]'; cmin = cmin(:)';
cmax = [1*ones(size(Npeak)),b0*Npeak+b0*(Npeak1-Npeak)*0.6,b0*3*ones(size(Npeak))]'; cmax = cmax(:)';
...
xlim2 = [0,1];
ylim2 = [0,0.15;0,0.05;0,0.05];
EL_range0 = [0,0.5;0,0.6;0,1];
beta00 = {[0.02,0.15,0.05,0.02,0.45,0.05,0],[-0.05,0.4,0.03,0],[-0.05,0.7,0.03,0]};
