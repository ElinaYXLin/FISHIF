...
for list_I = 4%N1
    ...
    for list_J = 2%1:M1%eval(run_list{list_I})
...
%         N_cycle = round(log2(max(mask_stack(:))))+2;   %%% Calculate the nuclear cycle number
        ...
        [nucleus_protein_profile,cytoplasmic_protein_profile,quanti_p,nucleus_protein_ab,flip_EL] = protein_profile3D(nucleus_DAPI_profile,max_image,EL_info,protein_channel,mask_stack,signal_stack,image_folder,N_cycle,resolution,resolutionz,[],[],[],[],[],[],flip_axis,EL_range);
