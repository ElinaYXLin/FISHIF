function [foci_data,fake_data,h,t_absolute,varargout] = dual_local_chop(foci_bw00,max_image00,SS,foci_layer,mask_stack,signal_stack,RNA_stack,nucleus_protein_profile,image_folder,N_cycle,varargin)
...
for ir = 1:length(r_size)
    ...
    foci_bw00 = foci_raw;
    foci_layer = layer_raw;
    fake_bw00 = false(size(foci_bw00));
    fake_layer = zeros(size(foci_bw00));
    bwi = logical(max(mask_stack,[],3));
    bw0 = bwlabel(imerode(bwi,strel('disk',r_edge)));
    nucleus_prop = regionprops((~imdilate(foci_bw00,strel('disk',2*r_size(ir)))).*bw0,'Area','PixelIdxList');
    bwf = bw0.*foci_bw00;
    ...
    for I_f = 1:length(f_index)
         try 
            if nucleus_prop(bwf(f_index(I_f))).Area > 1
                tcontinue = true;
                while tcontinue
...
%     end
    foci_data{ir} = zeros(sum(sum(foci_bw00)),5);   %%% foci data matrix initialization
    fake_data{ir} = zeros(sum(sum(fake_bw00)),5);   %%% control data matrix initialization
    %%% =======================================================================
    ...
    f_area = zeros(size(signal_stack));   %%% Filtered area
    for I_ind = 1:length(x_all)
%        try
            f_signal(x_all(I_ind),y_all(I_ind),z_layer(x_all(I_ind),y_all(I_ind))) = sum(sum(signal_stack(x_all(I_ind)-x_h:x_all(I_ind)+x_h,y_all(I_ind)-y_h:y_all(I_ind)+y_h,z_layer(x_all(I_ind),y_all(I_ind))).*bw01(x_all(I_ind)-x_h:x_all(I_ind)+x_h,y_all(I_ind)-y_h:y_all(I_ind)+y_h).*h{ir}));
            ...
            f_area(x_all(I_ind),y_all(I_ind),z_layer(x_all(I_ind),y_all(I_ind))) = sum(sum(bw01(x_all(I_ind)-x_h:x_all(I_ind)+x_h,y_all(I_ind)-y_h:y_all(I_ind)+y_h).*h{ir}));
%         catch err
%             I_ind
%             ir
%         end    
    ...
    f_area = f_area.*mask_stack;
    f_area0 = max(f_area,[],3);
    foci_data{ir}(:,4) = f_signal0(foci_bw00);
    fake_data{ir}(:,4) = f_signal0(fake_bw00);
    foci_data{ir}(:,5) = r_signal0(foci_bw00);
    fake_data{ir}(:,5) = r_signal0(fake_bw00);
    foci_data{ir}(:,3) = f_area0(foci_bw00);
    fake_data{ir}(:,3) = f_area0(fake_bw00);
    ...
    for I_layer = 1:size(signal_stack,3)
        nu_prop = regionprops(imerode(bwi, strel('disk',r_size(ir)+r_edge)).*bwn,signal_stack(:,:,I_layer),'MeanIntensity');
    ...
    foci_data{ir}(:,1) = max_image00(foci_bw00).*SS(foci_bw00);
    foci_data{ir}(:,2) = nucleus_protein_profile((bwn(foci_bw00)+1),2);
    foci_data{ir}(:,3) = foci_data{ir}(:,3).*mean3_nu(foci_bw00);

    fake_data{ir}(:,1) = max_image00(fake_bw00).*SS(fake_bw00);
    fake_data{ir}(:,2) = nucleus_protein_profile((bwn(fake_bw00)+1),2);
    fake_data{ir}(:,3) = fake_data{ir}(:,3).*mean3_nu(fake_bw00);
    %% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
