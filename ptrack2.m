function [peak_parameter,varargout] = ptrack2(imstack0,imstack,mask_out,varargin)
tic
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% %         fit_range0 = zeros(0,3);
        while Jfit
            ...
            Jpeak = find(any(rx <= 1.5*xrange & ry <= 1.5*yrange & rz <= 2*zrange,2));
            ...
            if ~isempty(X02D)
                ...
                Jpeak2D = find(any(rx2D <= 1.5*xrange & ry2D <= 1.5*yrange & rz2D <= 2*zrange,2));
...
% %         lb02D(:,[3,5]) = [X02D(Nfit2D)-xrange,Y02D(Nfit2D)-yrange];
        ...
        if size([xf0;xf02D],1) <= 10 && ~isempty([xf0;xf02D]) && ~isempty(xdata) && ~isempty(ydata)
...
% % %         exstatus(Nfit) = (exitflag > 0) & (exitflag1 > 0) & ((xf(:,2)+xf(:,4)) >= 0) & (4*xf(:,2).*xf(:,4) >= xf(:,6).*xf(:,6)) & (xf(:,3) > 0) & (xf(:,3) < dim0(1)) & (xf(:,5) > 0) & (xf(:,5) < dim0(2));% & (xf1(:,3) > 0) & (xf1(:,3) < dim0(3));   %%% exit status
        exstatus(Nfit) = (exitflag > 0) & (exitflag1 > 0) & ((xf(:,2)+xf(:,4)) >= 0) & (4*xf(:,2).*xf(:,4) >= xf(:,6).*xf(:,6));% & (xf1(:,3) > 0) & (xf1(:,3) < dim0(3));   %%% exit status
