...
mask_folder = 'masks/';
mask_name = 'mask.mat';
hist_folder = 'Histogram_nullo/';
fit_folder = 'Histogram_default/';
fit_add = '_spot_fit';
hist_tail = '_raw.xls';
N_thresh = 3;
output_tail = '.xls';
...
for list_I = 1:1%N1
    ...
    for list_J = 4:4%M1
        ...
        [signal_stack,signal2_stack] = stack3D(imclearborder(seg_bw),protein_channel,DAPI_channel,signal2_channel,image_folder,protein_signal2_mismatch);   %%% load 3D image stacks
        
        [foci_bw3D,max_image00,SS] = modified_foci3D([folder_list{list_I,1},hist_folder,sub_list{list_J,3}(1:(end-1)),hist_tail],max_image,signal2_channel,mask_stack,N_cycle,image_folder,protein_signal2_mismatch,Inten_thresh);
        [nucleus_signal2_profile,foci_signal2_profile,cytoplasmic_signal2_profile] = RNA_profile3D(mask_stack,cyto_bw,foci_bw3D,max_image00,SS,signal2_stack,resolution,image_folder,N_cycle);   %%% Input the foci area matrix from modified_foci.m if gaussian fitting method is used
        ...
        [foci_data,fake_data,h,t_absolute,r_size] = dual_local_local3D2(foci_bw00,max_image_list,mask_stack,signal_stack,signal2_stack,nucleus_protein_profile_ab,image_folder,N_cycle,resolution,resolutionz);
        clear mask_stack signal_stack signal2_stack nucleus_protein_profile_ab

        dual_profile(nucleus_protein_profile,nucleus_signal2_profile,foci_signal2_profile,image_folder,N_cycle);
...
%%% Output: %%%============================================================
        ...
        saveas(75,[result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),protein_add,local_add,signal2_add,D3_add,figure_tail]);
        ...
        %save([result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),mat_tail],'image_folder','image_type','WGA_channel','DAPI_channel','signal2_channel','protein_channel','resolution','seg_bw','cyto_bw','max_image','foci_bw','nucleus_signal2_profile','foci_signal2_profile','cytoplasmic_signal2_profile','nucleus_protein_profile','cytoplasmic_protein_profile','N_cycle','foci_data','fake_data','h','t_absolute','-append');
        save([result_folder,sub_list{list_J,3}(1:(length(sub_list{list_J,3})-1)),mat_tail],'image_folder','image_type','WGA_channel','DAPI_channel','signal2_channel','protein_channel','resolution','seg_bw','cyto_bw','max_image','nucleus_signal2_profile','foci_signal2_profile','cytoplasmic_signal2_profile','nucleus_protein_profile','cytoplasmic_protein_profile','N_cycle','foci_data','fake_data','h','t_absolute','r_size','resolutionz','quanti_p','-append');
        ...
        clear ('seg_bw','cyto_bw','max_image','foci_bw','nucleus_signal2_profile','foci_signal2_profile','cytoplasmic_signal2_profile','nucleus_protein_profile','cytoplasmic_protein_profile','N_cycle','foci_data','fake_data');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%        
