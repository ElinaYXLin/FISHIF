...
I_plot = [1:7,9:10,12];%[1:3,5:7,9:10];%[1,2,5,8,9,10];%[3:5,8:11,22:25,27:34];%[3:5,8:11,14:19,22:25,27:34];
I_cycle = 10:12;
epsilon = 1e-4;
smin = 0.1;
smin2 = 0.99;
...
% %     id0 = C_RNA0(:,IC) > 30e-9 & en_ring_ratio > smin;
    id0 = EL_RNA0 >= 0 & EL_RNA0 <= 0.5 & en_ring_ratio > smin;
    id1 = EL_RNA1 >= 0 & EL_RNA1 <= 0.5;
    xdata = dxy0(id0);
...
% %     id0 = C_RNA0(:,IC) < 5e-9 & en_ring_ratio > smin;
    id0 = EL_RNA0 >= 0.5 & EL_RNA0 <= 1 & en_ring_ratio > smin;
    id1 = EL_RNA1 >= 0.5 & EL_RNA1 <= 1;
    xdata = dxy0(id0);
    ydata = 1./en_ring_area(id0)./nnz(id1)/resolutionz/length(dz0);%./length(xdata);
...
sub_pos2 = [5,10];
Nbin = prod(sub_pos2); Nbin2 = 12; dN0 = 2;
rover = 0.4;
...
b0 = 1;
c0 =[0.5*10.^(-Npeak),b0*Npeak,b0/4*ones(size(Npeak))]'; c0 = c0(:)';
cmin = [zeros(size(Npeak)),b0*Npeak-b0*(Npeak-Npeak0)*0.5,b0/10*ones(size(Npeak))]'; cmin = cmin(:)';
cmax = [1*ones(size(Npeak)),b0*Npeak+b0*(Npeak1-Npeak)*0.5,b0*1*ones(size(Npeak))]'; cmax = cmax(:)';
...
% %     dscale0 = sum((curve_coef0(2:3:end)-Npeak').*curve_coef0(1:3:end).^1.*curve_coef0(3:3:end).^1)/sum(curve_coef0(1:3:end).^1.*curve_coef0(3:3:end).^1);
...
yyaxis left
    plot(EL_mean(1:2:end),Pon(1:2:end),'.k','DisplayName','Data')
hold on
    errorbar(EL_mean(1:2:end),Pon(1:2:end),Pon_err(1:2:end),'k','LineStyle','none','DisplayName','Error')
...
%     beta0 = [-5,0.6,0.1,0];
% %     beta0 = [-0.05,0.2,0.05,-0.05,0.6,0.3,0];
    beta0 = [-0.05,0.6,0.3,0];
% %     beta0 = [0.1,0.6,0.05,0];
% % %     beta_min = zeros(size(beta0)); %beta_min(1) = -100; 
% % % % %     beta_max = 2*beta0; beta_max(1) = 0; %beta_max(end) = 1;
% % %     beta_max = [1,1,1,0];
    beta_min = [-1,0,0,0];
    beta_max = [1,1,1,1];
% %     beta_min = [-1,0,0,-1,0,0,0];
% %     beta_max = [1,1,1,1,1,1,0];
    I_true0 = EL_mean >= 0.35 & EL_mean <= 0.7;
...
yyaxis right
    expf3 = @(beta,x) beta(3)*exp(-(x-beta(2))./beta(1))./(1+exp(-(x-beta(2))./beta(1)))+beta(4);
    beta3 = [-0.1281,0.4112,69.3004,0];
    plot(EL_line,expf3(beta3,EL_line),'b-','DisplayName','Hb');
ylabel('Hb concentration (nM)','FontName',nfont,'FontSize',fsize,'FontWeight','normal')
legend('show')
xlim(xlim5)
% % ylim(ylim51)
...
b0 = 1;
c0 =[0.1*ones(size(Npeak)),b0*Npeak,b0/3*ones(size(Npeak))]'; c0 = c0(:)';
cmin = [zeros(size(Npeak)),b0*Npeak-b0*(Npeak-Npeak0)*0.5,b0/8*ones(size(Npeak))]'; cmin = cmin(:)';
cmax = [2*ones(size(Npeak)),b0*Npeak+b0*(Npeak1-Npeak)*0.5,b0*ones(size(Npeak))]'; cmax = cmax(:)';
...
% % % % % Icut_min = [0,2,5,11,15];
% % % % % Icut_max = [2,5,11,15,20];
...
xlim2 = [0,1];
ylim2 = [0,0.2;0,0.2;0,0.1;0,0.06;0,0.06];
EL_range0 = [0,1;0,1;0,0.62;0,0.62;0,0.64];
% beta00 = {[0.01,0.3,0.1,0],[0.02,0.3,0.02,-0.02,0.8,0.02,0],[-0.02,0.5,0.02,0]};
beta00 = {[0.4,0.3,0.3,0],[0.1,0.3,0.2,0],[-0.02,0.3,0.05,0],[-0.02,0.3,0.05,0],[-0.02,0.3,0.05,0]};
% % beta00 = {[0.1,0.2,0.1,0.1,0.4,0.1,0],[0.1,0.3,0.2,0],[-0.02,0.3,0.05,0],[-0.02,0.3,0.05,0],[-0.02,0.3,0.05,0]};
...
for ii = 1:length(plotN)
    axes('Units','inches','Position',[dsize2+(dsize2+asize4).*[0,ii-1],asize4]);
        plot(xdata2,ydata2(:,plotN{ii}),'k.')
        hold on
        if plotN{ii} == 10
            g0 = fittype( @(h1,c1,a1,h2,c2,a2,d, x) a1*exp(-(x-c1)./h1)./(1+exp(-(x-c1)./h1))+a2*exp(-(x-c2)./h2)./(1+exp(-(x-c2)./h2))+d );
            beta0 = beta00{ii};
            beta_min = [-1,0,0,-1,0,0,0]; %beta_min(1) = -100; 
            beta_max = [1,1,1,0,1,1,1];
        else
            g0 = fittype( @(h1,c1,a1,d, x) a1*exp(-(x-c1)./h1)./(1+exp(-(x-c1)./h1))+d );
            beta0 = beta00{ii};
            beta_min = [-1,0,0,0];
        ...
        curve = fit(xdata2(I_true0),ydata2(I_true0,plotN{ii}), g0, 'StartPoint',beta0,'Lower',beta_min,'Upper',beta_max);
        plot(EL_line,feval(curve,EL_line),'r-','DisplayName','Fit');
    xlim(xlim2)
    ylim(ylim2(ii,:))
