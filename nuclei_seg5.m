function [seg_bw,cyto_bw,max_image,N_cycle,WGA_th0,DAPI_th0,fWGA] = nuclei_seg5(image_folder,image_type,WGA_channel,DAPI_channel,resolution)
%clear
...
for I_layer = 1:length(layer_name)
    ...
    temp_WGA = imtophat(temp_image(:,:,WGA_channel), se);
    fH = imfilter(double(temp_WGA) ,H,'replicate','conv');
    fH(fH < 0) = 0;
