function [nucleus_profile,cytoplasmic_profile,quanti_p,varargout] = protein_profile3D(nucleus_DAPI_profile,max_image,EL_info,signal_channel,mask_stack,signal_stack,image_folder,N_cycle,resolution,resolutionz,varargin)
...
for I_layer = 1:size(mask_stack,3)
    sg_prop = regionprops((imerode(logical(mask_stack(:,:,I_layer)), strel('disk',r_edge))).*mask_stack(:,:,I_layer),signal_stack(:,:,I_layer),'MeanIntensity','PixelValues');
    sg2_prop = regionprops((imerode(logical(mask_stack(:,:,I_layer)), strel('disk',r_edge))).*mask_stack(:,:,I_layer),signal_stack(:,:,I_layer).^2,'MeanIntensity');
    if I_layer == 1
...
% % %     end
    ...
    if xor(mean(Inten(nucleus_distance >= 0.5)) > mean(Inten(nucleus_distance <= 0.5)),flip_axis)
...
for Ic = 1:size(ijk,1)
    ...
    cytoplasmic_var(Ic) = var(sig0(repmat(cmask,[1,1,size(nmask,3)]) & (~nmask)));
end
...
figure(n3)
    ...
    hold on
    plot(nucleus_profile(index_true,2),fluc_profile(index_true,2),'g.')
    x0 = xlim;
    plot(x0,p(1)*(x0+p_post(2)/p_post(1)),'r')
    
    plot(cytoplasmic_profile(:,2),cytoplasmic_profile(:,3),'k.')
    plot(cytoplasmic_profile(index_true,2),cytoplasmic_profile(index_true,3),'c.')
    plot(x0,pc(1)*(x0+pc(2)/pc(1)),'m')
    hold off
    ...
    legend('Nuclear data points','Nuclear data points cleaned','Nuclear linear fit','Cytoplasmic data points','Cytoplasmic data points cleaned','Cytoplasmic linear fit')
%%% =======================================================================
