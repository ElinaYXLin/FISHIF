function Dual_all_add2(total_name)
...
%         [max_C,Imax] = max(nucleus_protein_profile_ab(:,2));
%         layer_max = nucleus_protein_profile(Imax,4);
%         max_S = nnz(mask_stack(:,:,layer_max) == Imax)*resolution^2;

%         out_name = cat(1,out_name,{image_folder});
%         out_data = cat(1,out_data,[N_cycle,max_C,max_S,max_C*max_S,layer_max]);

        [C0,IX] = sort(nucleus_protein_profile_ab(:,2),'descend');
        CC = zeros(1,10);
        SS = zeros(1,10);
        II = zeros(1,10);
        for ii = 1:10
            CC(ii) = C0(ii);
            II(ii) = nucleus_protein_profile(IX(ii),4);
            SS(ii) = nnz(mask_stack(:,:,II(ii)) == IX(ii))*0.083^2;
        end
        max_C = CC(1);
        max_S = max(SS);
        [max_CS,ICS] = max(CC.*SS);
        max_C0 = CC(ICS);
        max_S0 = SS(ICS);
        layer_max = II(ICS);
        
        out_name = cat(1,out_name,{image_folder});
        out_data = cat(1,out_data,[N_cycle,max_C,max_S,max_C0,max_S0,max_CS,layer_max]);
    end
end
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

out_data_all = cat(2,out_name,num2cell(out_data));
xlswrite([result_folder0,total_name,con_add,output_tail],out_data_all);
