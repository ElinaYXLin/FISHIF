...
I_plot = 1:42;%[3:5,8:11,22:25,27:34];%[3:5,8:11,14:19,22:25,27:34];
I_cycle = 12;
epsilon = 1e-4;
smin = 1e-1;
z_dim = 50;
...
% %     id0 = C_RNA0(:,IC) > 30e-9 & en_ring_ratio > smin;
    id0 = EL_RNA0(:,IC) <=0.3 & EL_RNA0(:,IC) >=0.15 & en_ring_ratio > smin;
    xdata = dxy0(id0);
...
% %     id0 = C_RNA0(:,IC) < 5e-9 & en_ring_ratio > smin;
    id0 = EL_RNA0(:,IC) >= 0.7 & EL_RNA0(:,IC) <= 0.85 & en_ring_ratio > smin;
    xdata = dxy0(id0);
    ydata = 1./en_ring_area(id0)./length(xdata);
...
EL0 = [0,1];
N_range = 0;
bin_min = 0:1:100;   %%% bin_min for [Bcd]
bin_max = 5:1:105;   %%% bin_max for [Bcd]
C_range_mean0 = [0,9];
